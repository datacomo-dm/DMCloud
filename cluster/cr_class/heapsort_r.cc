...
#define CREATE(initval, nmemb, par_i, child_i, par, child, size, count, tmp) { \
	for (par_i = initval; (child_i = par_i * 2) <= nmemb; \
		...
		if (child_i < nmemb && compar(child, child + size, compar_arg) < 0) { \
			child += size; \
			++child_i; \
		} \
		par = base + par_i * size; \
		if (compar(child, par, compar_arg) <= 0) \
			break; \
...
#define SELECT(par_i, child_i, nmemb, par, child, size, k, count, tmp1, tmp2) { \
	...
	for (;;) { \
		...
		child = base + child_i * size; \
		par = base + par_i * size; \
		if (child_i == 1 || compar(k, par, compar_arg) < 0) { \
 ...
 */
int
CR_Class_NS::heapsort_r(void *vbase, size_t nmemb, size_t size, sort_cmp_func_t compar, void *compar_arg)
{
	size_t cnt, i, j, l;
	char tmp, *tmp1, *tmp2;
	char *base, *k, *p, *t;
