...
int
do_nd_job_other_req(void *server_private_p, void *connect_private_p,
    ...
    int8_t &cmd_code_to_client, std::string &data_to_client, std::string &errmsg)
{
    ...
    std::string data_from_job;
    CFMJobExecutorMap::JobExecutorInfo *je_p;

    je_p = je_map_p->get_job(job_id);
    if (je_p) {
        ret = je_p->job_conn.SendAndRecv(
          cmd_code_from_client, &data_req, cmd_code_to_client, &data_from_job);
        je_map_p->put_job(je_p);
        if (!ret) {
            if (cmd_code_to_client == CFM_ERROR_RESP) {
                data_to_client = data_from_job;
            } else {
                CFM_Basic_NS::exchange_msg_merge(job_id, &data_from_job, data_to_client);
            }
        }
    } else
        ret = ENOENT;
