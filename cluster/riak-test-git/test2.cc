...
#define TREE_ADD_COUNT		(20)
#define TREE_POP_COUNT		(40)
#define QUEUE_PUSH_COUNT	(20)
#define QUEUE_PULL_COUNT	(40)
...
int main(int argc, char *argv[])
{
	...
	for (int i=0; i<TREE_ADD_COUNT; i++) {
		...
		tdb.default_tags_str["tree"] = "global_tree";
	...
	while (!senders.empty()) {
		...
		cr_mt->Stop("SEND", thread_name, 10);
		DPRINTF("Kill %s\n", thread_name.c_str());
		senders.pop_front();
	}
	...
	while (!treepop.empty()) {
		...
		cr_mt->Stop("TREE_POP", thread_name, 10);
		DPRINTF("Kill %s\n", thread_name.c_str());
		treepop.pop_front();
	}
	...
	while (!queuepull.empty()) {
		...
		cr_mt->Stop("QUEUE_PULL", thread_name, 10);
		DPRINTF("Kill %s\n", thread_name.c_str());
		queuepull.pop_front();
	}
...
tree_add(CR_Thread::handle_t thread_self)
{
	...
	cmt_p = (CR_MultiThread *)CR_Thread::gettag_ptr(thread_self, CR_MULTITHREAD_TAG_MULTITHREAD_PTR);
	tree_p = cmt_p->GetTreePtr(CR_Thread::gettag_str(thread_self, "tree"));
...
tree_pop(CR_Thread::handle_t thread_self)
{
	...
	while (1) {
		...
		if ((err = tree_p->PopOneScoped("K-", "L", k, v, NULL, 1000000)) == 0) {
...
queue_push(CR_Thread::handle_t thread_self)
{
	...
	while (1) {
		...
		if ((err = cmt_p->PushToThreadQueueByHash("QUEUE_PULL", k, v)) == 0) {
			DPRINTF("PushToThreadQueueByHash K=%s, V=%s OK\n", k.c_str(), v.c_str());
		} else {
			DPRINTF("PushToThreadQueueByHash K=%s, V=%s Failed, err:%s\n", k.c_str(), v.c_str(), strerror(err));
		}
...
queue_pull(CR_Thread::handle_t thread_self)
{
	...
	while (1) {
		...
		if ((err = CR_Thread::queue_pull(thread_self, v, 1000000)) == 0) {
			DPRINTF("PullFromQueue OK, got %s\n", v.c_str());
		} else {
			DPRINTF("PullFromQueue Failed, err:%s\n", strerror(err));
		}
	}
}
...
on_thread_exit(CR_Thread::handle_t thread_self, const std::string &result)
{
	...
	t_class = CR_Thread::gettag_str(thread_self, CR_MULTITHREAD_TAG_THREAD_CLASS);
	t_id = CR_Thread::gettag_str(thread_self, CR_MULTITHREAD_TAG_THREAD_ID);

	DPRINTF("thread %s[%s](%016llX) sad \"%s\" and exit\n",
		t_class.c_str(), t_id.c_str(), (long long unsigned int)thread_self, result.c_str());
...
on_thread_cancel(CR_Thread::handle_t thread_self)
{
	...
	DPRINTF("thread %s[%s](%016llX) CANCELED\n",
		t_class.c_str(), t_id.c_str(), (long long unsigned int)thread_self);
