				    ...
				    LEX_STRING *table_name)
{
  ...
  else
  {
    ...
    if(file)
    {
	  ...
	  // GA: table comment update restored to provide compression ratio
	  char *comment = show_table->file->update_table_comment(share->comment.str);
	  if (comment)
	  {
      #if 0
      // delete by liujs: fix dma-2015 , comment begin with : 0x0871
		  table->field[20]->store(comment,
			  (comment == share->comment.str ? share->comment.length : strlen(comment)), cs);
      #else
      const char* _ib_comment_begin_pos = "0x0871";
      char *ib_comment_begin = strstr(comment,_ib_comment_begin_pos);
      if(ib_comment_begin){
           table->field[20]->store(ib_comment_begin,strlen(ib_comment_begin), cs);
      }
      #endif
