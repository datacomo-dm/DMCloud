...
extern char infobright_version[];
...
static void set_ports()
{
  ...
  {					// Get port if not from commandline
    ...
    /*
      ...
      on that, fall back to the factory default of 5029).
    ...
    */
    ...
    if ((serv_ptr= getservbyname("infobright", "tcp")))
...
err:
  sql_print_error("Fatal error: Can't change to run as user '%s' ; \
Please check that the user exists! %s\n", user, errno ? (const char *)strerror(errno) : "");
  unireg_abort(1);
				 ...
				 char **argv, const char **groups)
{
  ...
  */
  ...
  if (get_options(&defaults_argc, defaults_argv))
    return 1;
  /* Infobright! We need those two options on our side. */
  infobright_home_dir = mysql_home;
  infobright_data_dir = mysql_real_data_home;
  if ( ! infobright_data_dir.empty() ) {
	  int len = infobright_data_dir.length();
	  if ( infobright_data_dir[ len - 1 ] != '/' )
		  infobright_data_dir += "/";
  } else
	  infobright_data_dir += "/";
  if ( ! infobright_home_dir.empty() ) {
	  int len = infobright_home_dir.length();
	  if ( infobright_home_dir[ len - 1 ] != '/' )
		  infobright_home_dir += "/";
  } else
	  infobright_home_dir += "/";
  set_server_version();
...
we force server id to 2, but this MySQL server will not act as a slave.");
  ...
  */
  ...
  sql_print_information("%s is ready for connections.\nsocket: '%s' port: %d %s",
						infobright_version,
...
thread is in the master's binlogs.",
  ...
  {"master-port", OPT_MASTER_PORT,
   "The port the master is listening on. If not set, the compiled setting of MYSQL_PORT is assumed. If you have not tinkered with configure options, this should be 5029. The value in master.info will take precedence if it can be read.",
...
#include <sslopt-case.h>
  ...
  /*
    ...
    configuration is specified either in the my-ib.cnf file or on
  ...
  */
  ...
  case OPT_LOW_PRIORITY_UPDATES:
    ...
    break;
  case OPT_BOOTSTRAP:
    opt_noacl=opt_bootstrap=infobright_bootstrap=1;
    break;
  case OPT_SERVER_ID:
...
#ifndef EMBEDDED_LIBRARY
  process_type = ProcessType::MYSQLD; // Infobright
  if (mysqld_chroot)
    set_root(mysqld_chroot);
#else
  process_type = ProcessType::EMBEDDED;  // Infobright
