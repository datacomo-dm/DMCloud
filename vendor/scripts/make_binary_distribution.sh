...
if [ x"$BASE_SYSTEM" != x"netware" ] ; then
  ...
  [ -f scripts/mysqlaccess.conf ] && cp scripts/mysqlaccess.conf $DEST/bin/

  copyfileto $DEST/support-files support-files/*
  copyfileto $DEST/bin/ sql/bhloader sql/mysqld sql/updater sql/chmt 
  [ -f sql/icm ] && copyfileto $DEST/bin/icm sql/icm

  # FIXME, some hack. need to solve this copy operation by Makefile
  copyfileto $DEST/lib/mysql/ libmysqld/libbrighthouse* \
    ./libcsv.* ./libheap.* ./libmyisam.* ./libmyisammrg.* ./libregex.* \
    ./libsql.* ./libtaocrypt.* ./libvio.* ./libyassl.*
...
copyfileto $BASE COPYING COPYING.LIB README
