...
parse_arguments()
{
  ...
  do
    case "$arg" in
      ...
      --cachedir=*) cachedir=`parse_arg "$arg"` ;;
      --user=*)
...
test -z "$cachedir" && cachedir="$basedir/cache"
for dir in $ldata $ldata/mysql $ldata/test $ldata/sys_infobright $cachedir
do
  if test ! -d $dir
  then
    mkdir -p $dir
