...
public: 
	int Run();
private:
	std::pair<int, int>	 MigrateCS(const string& path_to_frm);
	void PrepareCharsetsToUpdate(fstream& frm_file);
	std::pair<int, int> UpdateCharsets();
	void StoreCharsets(fstream& frm_file);
	int IsMigrationNeeded(const string& path_to_frm);
...
private:
	...
	std::vector<std::pair<fstream::off_type, uchar> > charsets_to_update; //
};
