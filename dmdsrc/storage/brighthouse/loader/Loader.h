...
struct LoaderParams
{
	...
	/* add connect id for session identify since client of mysql can not get session id to bhloader
	*/
	ulong connect_id;
...
public:
	...
	ulong &ConnectID() {return params.connect_id;}
...
//STATIC
public:
	static void BuildId(std::string& id);
...
private:
	static IBMutex localid_mutex;
	static uint64 cur_localid;
};
