...
int main(int argc, char *argv[])
{
  ...
  /* DT Project modified */
  /* keep parameter control file for debug */
  char *keep_ctl_dbg=getenv("DP_KEEP_CTL_DBG");
  if(keep_ctl_dbg!=NULL) {
  	char cmd[300];
  	snprintf(cmd, sizeof(cmd),"cp -f %s /tmp/bhd_%s",argv[2],argv[1]);
  	system(cmd);
  }
  /* DT Project modified end */
