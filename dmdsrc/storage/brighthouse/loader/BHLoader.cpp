...
//#ifndef RCLOAD_STANDALONE
	char *loadmethod=getenv("DP_LOADINPROC_DEBUGONLY");
	if(loadmethod!=NULL && strcmp(loadmethod,"1366")==0) {
...
//#endif
	try {
    	...
    	rclog << lock << str(format("System error(%1%) on bhloader execution: %2%.") % e.GetErrorCode() % GetErrorMessage(e.GetErrorCode())) << unlock;
    } catch(...) {
        shared_file.Close();
...
int LoadData(string shared_obj_name, string shared_obj_path)
{
...
#ifndef NDEBUG
		//sleep waiting debug
		// modify waittime value in debug tool to continue
		int waittime=20;
		for(int wd=0;wd<waittime;wd++)
			sleep(1);
#endif
	...
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
			...
			for(int i = 0; i < bhl.GetIOParameters().ColumnsCollations().size(); i++) {
           	...
           	rct = RCTableLoadAutoPtr(new RCTableLoad(table_path.c_str(), 1, colls)); // syncode dma-1329
		...
		if (rct->GetSaveSessionId() != 0xFFFFFFFF && rct->GetSaveSessionId() != transaction_id)	{
			...
			try {
				rct->LoadData(bhl.GetIOParameters(),(uint)bhl.ConnectID());
