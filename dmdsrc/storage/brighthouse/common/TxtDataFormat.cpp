...
{
	// use iop.outpath to decide binary file process or text file process.
	const char*	path=iop.Path();
	// parser not needed(for insert row by sql)
	if(!*path) 
		return std::auto_ptr<DataParser>(NULL);
	if(strcmp(path+strlen(path)-4,".bin")==0) {
	   //simpbin=true;
	   DataParser *dpr=new DataParserForBinary(attrs, buffer, iop, mpr);
	   dpr->SetSimpBin(true);
	   return std::auto_ptr<DataParser>(dpr);
	   //return std::auto_ptr<DataParser>(new DataParserForBinary(attrs, buffer, iop, mpr));
	}
