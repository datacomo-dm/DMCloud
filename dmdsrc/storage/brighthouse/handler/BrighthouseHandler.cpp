...
int BrighthouseHandler::open(const char *name, int mode, uint test_if_locked)
{
	...
	int ret = 1;
	SetConnectInfo(rceng->GetThreadParams(*table->in_use));
...
int BrighthouseHandler::close(void)
{
	...
	try {
		// the last use reference,then clear memory.
		if(share->use_count==1){
		    // release dpn&rsi memory
		    if(m_conn) {
    			m_conn->GetTransaction()->GetTable(m_table_path, table_share)->ReleaseAttributes();
		    }else{
		        rceng->GetTableShared(m_table_path, table_share)->ReleaseAttributes();
		    }
		}        
	} catch (std::exception& e) {
		my_message(BHERROR_UNKNOWN, e.what(), MYF(0));
...
void BrighthouseHandler::ResetConnectionInfo(ConnectionInfo *pconn) {
    if(m_conn==pconn)
        m_conn=NULL;
}

void BrighthouseHandler::SetConnectInfo(ConnectionInfo *pconn) {
    if(m_conn==pconn) return;
    if(m_conn!=NULL)
        m_conn->RemoveHoldTable(this);
    m_conn=pconn;
    if(m_conn)
        m_conn->AddHoldTable(this);
}
	...
	try {
		if(m_query && !m_result && table_ptr->NoObj() != 0) {
			...
			try {
                ...
                // add bh.err log by liujs
                rccontrol.lock(m_conn->GetThreadID()) << " Query syntax not implemented in Brighthouse, executed by MySQL engine. " << unlock;

				TempTable* push_down_result = m_query->Preexecute(*m_cq, NULL);
				...
				if(!filter)
					filter_ptr = FilterAutoPtr(new Filter(push_down_result->GetMultiIndexP()->OrigSize(0),nfblocks));
				else
				...
				// if filter == NULL, what next line means?
...
int BrighthouseHandler::rnd_end()
{
    ...
    // add bh.err log by liujs
    rccontrol.lock(m_conn->GetThreadID()) << "----------------------------------------------------------------------------" << unlock;
...
//		table_new_iter.UpdateUsedAttrs(GetAttrsUseIndicator(table));
	...
	for(Field **field = table->field; *field; field++, col_id++) {
		if(bitmap_is_set(table->read_set, col_id)) {
			RCEngine::Convert(is_null, *field, (*field)->ptr, *(*table_new_iter)[col_id]);
			if(is_null == 0)
				(*field)->set_notnull();
			else {
...
extern int bh_sysvar_parscan_maxscanthreads;
...
static MYSQL_SYSVAR_INT(ib_parscan_maxscanthreads, bh_sysvar_parscan_maxscanthreads, PLUGIN_VAR_READONLY, "-", NULL, NULL, 0, 0, 512, 0);
...
static struct st_mysql_sys_var* bh_showvars[]= {
  ...
  MYSQL_SYSVAR(ib_parscan_maxscanthreads),
...
mysql_declare_plugin(datacomoap)
{
	MYSQL_STORAGE_ENGINE_PLUGIN,
	&brighthouse_storage_engine,
	"DATACOMOAP",
