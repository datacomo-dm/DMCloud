...
public:
	 ...
	 */
	ConstColumn(ValueOrNull const& val, ColumnType const& ct) : VirtualColumn(ct, NULL), value(val) {
		dim = -1;
	}
	...
	virtual bool RoughNullsOnly(const MIIterator &mit) { return value.IsNull(); }
...
protected:
	virtual BHReturnCode DoGetValueRCNum(MIIterator const&, RCNum& rc);
	virtual BHReturnCode DoGetValueDateTime(MIIterator const&, RCDateTime& dt, AttributeType t = RC_UNKNOWN);
	...
	virtual _int64 DoGetMinInt64(const MIIterator &mit) { return value.IsNull() ? NULL_VALUE_64 : value.Get64(); }
	virtual _int64 DoGetMaxInt64(const MIIterator &mit) { return value.IsNull() ? NULL_VALUE_64 : value.Get64(); }
	virtual _int64 DoRoughMin()	{ return value.IsNull() ? NULL_VALUE_64 : value.Get64(); }
	virtual _int64 DoRoughMax() { return value.IsNull() ? NULL_VALUE_64 : value.Get64(); }
