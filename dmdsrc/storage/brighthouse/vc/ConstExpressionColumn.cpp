...
void ConstExpressionColumn::RequestEval()
{
#ifdef PURE_LIBRARY
...
BHReturnCode ConstExpressionColumn::DoGetValueDateTime(MIIterator const&, RCDateTime& dt, AttributeType t) {
	BHReturnCode rc = BHRC_SUCCESS;

	assert(ATI::IsDateTimeType(TypeName()));
	if(t == RC_UNKNOWN)
		t= TypeName();
	dt = RCDateTime(last_val.GetDateTime64(), t);
	return rc;
}

BHReturnCode ConstExpressionColumn::DoGetValueRCNum(MIIterator const& mit, RCNum& rcn) {
	BHReturnCode rc = BHRC_SUCCESS;
	assert(ATI::IsNumericType(TypeName()));
	if(!IsNull(mit)) {
		rcn = RCNum(last_val.x, Type().GetScale(), Type().GetScale() == -1, TypeName());
	} else
		rcn = RCNum();
	return rc;
}
