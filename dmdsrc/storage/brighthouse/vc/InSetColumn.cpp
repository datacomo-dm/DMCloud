	...
	last_mit(NULL), last_mit_size(0)
{
	...
	for(columns_t::iterator it = columns.begin(), end = columns.end(); it != end; ++ it) {
		...
		MysqlExpression::SetOfVars params_ = (*it)->GetParams();
...
RSValue InSetColumn::DoRoughCheck(const MIIterator& mit, Descriptor& d)
{
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT( !"To be implemented." );
...
BHTribool InSetColumn::DoContains64(const MIIterator& mit, _int64 val)			// easy case for non-null numerics
{
	assert(val != NULL_VALUE_64);
	if(cache.EasyMode()) {
...
void InSetColumn::LockSourcePacks(const MIIterator& mit,Descriptor *pdesc)
{
	for(int i = 0; i < columns.size(); i++) {
		columns[i]->LockSourcePacks(mit,pdesc);
	}
}

void InSetColumn::LockSourcePacks(const MIIterator& mit,int th_no)
{
	for(int i = 0; i < columns.size(); i++) {
		columns[i]->LockSourcePacks(mit,th_no);
	}
}
