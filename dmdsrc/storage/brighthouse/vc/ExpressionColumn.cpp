...
RSValue ExpressionColumn::DoRoughCheck(const MIIterator& mit, Descriptor& d)
{
...
void ExpressionColumn::LockSourcePacks(const MIIterator& mit,Descriptor *pdesc)
{
	for (vector<VarMap>::iterator it = var_map.begin(); it != var_map.end() ; it++)
		it->tabp = it->GetTabPtr().get();
	VirtualColumn::LockSourcePacks(mit,pdesc);
	//pguard.LockPackrow(mit);		// TODO: do something with a return code, when it becomes nontrivial
}

void ExpressionColumn::LockSourcePacks(const MIIterator& mit,int th_no)
{
	for (vector<VarMap>::iterator it = var_map.begin(); it != var_map.end() ; it++)
		it->tabp = it->GetTabPtr().get();
	VirtualColumn::LockSourcePacks(mit,th_no);
	//pguard.LockPackrow(mit,NULL,th_no);		// TODO: do something with a return code, when it becomes nontrivial
}
