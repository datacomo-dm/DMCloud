...
{
	return col->MaxStringSize(mind->GetFilter(dim));
}
...
RSValue SingleColumn::DoRoughPackIndexCheck(const MIIterator& mit, Descriptor& d,std::vector<int> &packs)
{
	//TODO : anything
	//   iterator all of mit but not only current, and mix with leveldb result
	if(mit.GetCurPackrow(dim) >= 0) {
		// check whether isn't it a join
		SingleColumn* sc = NULL;
		if(d.val1.vc && d.val1.vc->IsSingleColumn())
		...
		{	
			MIUpdatingIterator *pmit=(MIUpdatingIterator *)&mit;
			return col->RoughPackIndexCheck(*pmit,dim, d, mit.NullsPossibleInPack(dim),packs);
		}
	} else
		return RS_UNKNOWN;
}
