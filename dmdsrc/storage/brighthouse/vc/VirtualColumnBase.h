...
public:
	 ...
	 */
	//inline void GetValueBin(const MIIterator& mit, int& size, char* buf) {
	//	return (DoGetValueBin(mit, size, buf));
	//}
	 ...
	 * \return numeric value from the column in level-1 encoding. or NULL_VALUE_64 if not min available
	 * - Integers values can be used directly
	 * - Decimals must be externally properly shifted
	 * - Doubles must be cast e.g. *(double*)&
	 */
	 ...
	 * 	\return min or NULL_VALUE_64 if min not known exactly
	 */
	 ...
	 * \return numeric value from the column in level-1 encoding. or NULL_VALUE_64 if not max available
	 * - Integers values can be used directly
	 * - Decimals must be externally properly shifted
	 * - Doubles must be cast e.g. *(double*)&
	 */
	 ...
	 * 	\return max or NULL_VALUE_64 if max not known exactly
	 */
	...
	/*! \brief True, if we can prove that the given data pack contains only nulls.
	*/
	virtual bool RoughNullsOnly(const MIIterator &mit) = 0;
	...
	*/
	...
	inline BHReturnCode GetValueRCNum(MIIterator const& mit, RCNum& rc) {
		return DoGetValueRCNum(mit, rc);
	}
	inline BHReturnCode GetValueDateTime(MIIterator const& mit, RCDateTime& dt, AttributeType t = RC_UNKNOWN) {
		return DoGetValueDateTime(mit, dt, t);
	}
	...
	virtual void RequestEval() {first_eval = true;} //TODO: checking if new parameter values are different form the previous ones
	...
	inline RSValue RoughPackIndexCheck(const MIIterator& it, Descriptor& d,std::vector<int> &packs) {
		return DoRoughPackIndexCheck(it, d,packs);
	}
...
protected:
	...
	virtual _int64 DoGetApproxSum(const MIIterator& mit, bool &nonnegative) { return DoGetSum(mit, nonnegative); }
	...
	virtual RSValue DoRoughCheck(const MIIterator&, Descriptor&) = 0;
	//Same like DoRoughCheck only implemented in TypeCastColumn && SingleColumn
	virtual RSValue DoRoughPackIndexCheck(const MIIterator&, Descriptor&,std::vector<int> &packs) {
		return RS_UNKNOWN; //not implemented
	};
	virtual void DoEvaluatePack(MIUpdatingIterator& mit, Descriptor&) = 0;
	virtual BHReturnCode DoGetValueRCNum(MIIterator const&, RCNum&) {
		BHASSERT_WITH_NO_PERFORMANCE_IMPACT(!"not implemented for this type of column");
		return BHRC_FAILD;
	}
	virtual BHReturnCode DoGetValueDateTime(MIIterator const& mit, RCDateTime& dt, AttributeType t = RC_UNKNOWN) {
		BHASSERT_WITH_NO_PERFORMANCE_IMPACT(!"not implemented for this type of column");
		return BHRC_FAILD;
	}
