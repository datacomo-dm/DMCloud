...
int
 ib_pack_assist::SnappyCode2IBCode(const int snappy_code)
{
	switch (snappy_code) {
	case SNAPPY_OK:
		return CPRS_SUCCESS;
...
namespace ib_packn_assist {
	template < typename etype >
	    int RemoveNullsAndCompressSnappy(uchar * tmp_comp_buffer,
					     uint & tmp_cb_len,
					     const _uint64 maxv,
					     const int no_nulls,
					     const uint * nulls,
		...
		size_t tmp_cb_len_local = tmp_cb_len;
		int rt = snappy_compress((const char *)tmp_data.get(),
					 ((no_obj - no_nulls) * sizeof(etype)),
					 (char *)tmp_comp_buffer,
					 &tmp_cb_len_local);
		if (SNAPPY_OK == rt)
			tmp_cb_len = tmp_cb_len_local;
		return ib_pack_assist::SnappyCode2IBCode(rt);
	}

	template < typename etype >
	    int DecompressAndInsertNullsSnappy(uint * &cur_buf,
					       const int no_nulls,
					       const uint * nulls,
					       const int no_obj,
					       void *data_full) {
		uint _buf_len = *cur_buf;
		size_t _tmp_dest_len = sizeof(etype) * no_obj;
		char *_pbuf = (char *)(cur_buf + 3);
		int rt = snappy_uncompress((const char *)_pbuf, _buf_len,
					   (char *)data_full, &_tmp_dest_len);
		if (SNAPPY_OK == rt) {
...
int ib_packn_compress_snappy(const uint * nulls,	/*: nulls 存储缓存      */
...
{				/*: 压缩状态选项        */
	...
	if (maxv != 0) {
		tmp_cb_len =
		    snappy_max_compressed_length((no_obj -
						  no_nulls) * value_type);
		if (value_type == UCHAR) {
			if (tmp_cb_len)
				tmp_comp_buffer =
			...
			retcode =
			    ib_packn_assist::RemoveNullsAndCompressSnappy <
	...
	if (no_nulls > 0) {
		if (ib_pack_assist::ShouldNotCompress(no_obj)) {
			...
			size_t _tmp_null_buf_size =
			    snappy_max_compressed_length(null_buf_size);
			comp_null_buf =
			    CR_MemGuard < uchar > (_tmp_null_buf_size);
			int rt = snappy_compress((const char *)nulls,
						 ((no_obj + 7) / 8),
						 (char *)comp_null_buf.get(),
						 &_tmp_null_buf_size);
			retcode = ib_pack_assist::SnappyCode2IBCode(rt);
			if (CPRS_SUCCESS == retcode) {
...
int ib_packn_decompress_snappy(const uchar * compressed_buf,	/*: 压缩后的缓存        */
...
{				/*: 数据包中对象数      */
	...
	if (no_nulls > 0) {
		...
		if (!ib_packn_compressmode_assist::IsModeNullsCompressed(optimal_mode)) {	// no nulls compression
			...
			size_t _tmp_no_nulls = no_nulls;
			int rt =
			    snappy_uncompress((const char *)((char *)cur_buf +
							     2),
					      null_buf_size,
					      (char *)nulls, &_tmp_no_nulls);
			retcode = ib_pack_assist::SnappyCode2IBCode(rt);
			if (CPRS_SUCCESS != retcode) {
	...
	if (!ib_packn_compressmode_assist::IsModeValid(optimal_mode)) {
		...
		if (ib_packn_compressmode_assist::
			...
			if (value_type == UCHAR) {
				retcode =
				    ib_packn_assist::
				    DecompressAndInsertNullsSnappy < uchar >
				    (cur_buf, no_nulls, nulls, no_obj,
				     data_full);
			} else if (value_type == USHORT) {
				retcode =
				    ib_packn_assist::
				    DecompressAndInsertNullsSnappy < ushort >
				    (cur_buf, no_nulls, nulls, no_obj,
				     data_full);
			} else if (value_type == UINT) {
				retcode =
				    ib_packn_assist::
				    DecompressAndInsertNullsSnappy < uint >
				    (cur_buf, no_nulls, nulls, no_obj,
				     data_full);
			} else {
				retcode =
				    ib_packn_assist::
				    DecompressAndInsertNullsSnappy < _uint64 >
				    (cur_buf, no_nulls, nulls, no_obj,
				     data_full);
			}
...
int ib_packs_compress_snappy(const uint * nulls,	/*: nulls 存储缓存      */
			     const int no_nulls,	/*: null 个数           */
...
{				/*: 压缩状态选项        */
	...
	if (no_nulls > 0) {
		if (ib_pack_assist::ShouldNotCompress(no_obj)) {
			...
			size_t _tmp_comp_null_buf_size =
			    snappy_max_compressed_length(comp_null_buf_size);
			comp_null_buf =
			    CR_MemGuard < uchar > (_tmp_comp_null_buf_size);
			int rt =
			    snappy_compress((const char *)nulls,
					    ((no_obj + 7) / 8),
					    (char *)comp_null_buf.get(),
	...
	if (maxv != 0) {
		...
		size_t _tmp_comp_len_buf_size =
		    snappy_max_compressed_length(comp_len_buf_size);
		comp_len_buf =
		    CR_MemGuard < uint >
		    (_tmp_comp_len_buf_size / sizeof(uint));
		int rt =
		    snappy_compress((const char *)nc_buffer.get(),
				    onn * sizeof(uint),
	...
	*(comp_len_buf.get() + 1) = maxv;	// the max length
	...
	dlen = snappy_max_compressed_length(data_full_byte_size) + 128;
	...
	if (data_full_byte_size) {
		...
		int rt =
		    snappy_compress((const char *)tmp_combination_index.get(),
				    index_pos,
				    (char *)comp_buf.get(), &_tmp_dlen);
		retcode = ib_pack_assist::SnappyCode2IBCode(rt);
...
int ib_packs_decompress_snappy(const uchar * compressed_buf,	/*: 压缩后的缓存        */
...
{				/*: null 个数           */
	...
	if (no_nulls > 0) {
		...
		if (!ib_packs_compressmode_assist::
			...
			size_t _tmp_null_buf_size = null_buf_size;
			int rt = snappy_uncompress((const char *)(cur_buf + 2),
	...
	if (optimal_mode == 0) {
		...
		if ((_uint64) * (cur_buf + 1) != 0) {	// maxv is not zeor
			...
			size_t _tmp_len = (1 << 16) * sizeof(uint);
			CR_MemGuard < uint > cn_ptr(1 << 16);
			int rt =
			    snappy_uncompress((const char *)(cur_buf + 2),
					      ((comp_len_buf_size -
						8) * sizeof(char)),
					      (char *)cn_ptr.get(), &_tmp_len);
			retcode = ib_pack_assist::SnappyCode2IBCode(rt);
			if (CPRS_SUCCESS != retcode) {
		...
		if (objs) {
			...
			size_t _tmp_data_full_byte_size = data_len;
			int rt = snappy_uncompress((const char *)cur_buf, dlen,
						   (char *)data,
