...
public:
	...
	uint				distcode; // distribution process identify code for this descriptor
	Descriptor();
    ...
    void SwitchSides();
	bool IsNull() const { return op == O_UNKNOWN_FUNC; }
    ...
    void EvaluatePack(MIUpdatingIterator& mit, _int64 *pahead);
	  void EvaluatePack(MIUpdatingIterator& mit);	// Assumption: no locking needed, done inside
	  RSValue EvaluateRoughlyPack(MIIterator& mit);
	  RSValue EvaluateRoughlyPackIndex(MIIterator& mit,std::vector<int> &packs);
...
struct DescTreeNode {
	...
	{ desc.attr = e1; desc.op = op; desc.val1 = e2; desc.val2 = e3; desc.like_esc = like_esc; desc.Simplify(true); desc.CoerceColumnTypes(); desc.CalculateJoinType(); }
