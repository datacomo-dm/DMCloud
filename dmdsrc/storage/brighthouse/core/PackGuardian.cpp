...
void VCPackGuardian::PreloadPack(const MIIterator &mit,std::vector<_int64 *> &prefetches) {
	if(!initialized)
		Initialize(mit,threads);
	int fct=0;
	int fsize=prefetches.size();
	for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++,fct++) {
		if(fsize<=fct) continue;
		JustATable* tab = iter->GetTabPtr().get();
		tab->PreloadPack(iter->col_ndx,prefetches[fct],my_vc.ConnInfo(),NULL);
	}
}

void VCPackGuardian::WaitPreload() {
	for(std::vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++) {
		JustATable* tab = iter->GetTabPtr().get();
		if(tab!=NULL)
			tab->WaitPreload(iter->col_ndx,my_vc.ConnInfo().GetTransaction()->GetID());
	}
}

PG_Status VCPackGuardian::LockPackrow(const MIIterator &mit,std::vector<_int64 *> &prefetches,int threadt,Descriptor *pdesc)
{
	if(!initialized)
		Initialize(mit,threads);
	int fsize=prefetches.size();
	if(current_strategy == LOCK_ONE) {
		...
		for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++,fct++) {
			...
			if(last_pack[cur_dim][threadt] != mit.GetCurPackrow(cur_dim)) {
				JustATable* tab = iter->GetTabPtr().get();
				if(last_pack[cur_dim][threadt] != NULL_VALUE_32)
					tab->UnlockPackFromUse(iter->col_ndx, last_pack[cur_dim][threadt]);
				try {
					void *pf=NULL;
					uint range=mit.GetRangeInPack(cur_dim,mit.GetCurPackrow(cur_dim),&pf);
					tab->LockPackForUse(iter->col_ndx,fsize>fct?prefetches[fct]:NULL, mit.GetCurPackrow(cur_dim),my_vc.ConnInfo(),pdesc,range,pf);
				} catch(...) {
					...
					for(; it != iter; ++it) {
						int cur_dim = it->dim;
						if(last_pack[cur_dim][0] != mit.GetCurPackrow(cur_dim) && last_pack[cur_dim][threadt] != NULL_VALUE_32)
							it->GetTabPtr()->UnlockPackFromUse(it->col_ndx, mit.GetCurPackrow(cur_dim));
					}

					for(++iter; iter != my_vc.GetVarMap().end(); ++iter) {
						int cur_dim = iter->dim;
						if(last_pack[cur_dim][threadt] != mit.GetCurPackrow(cur_dim) && last_pack[cur_dim][threadt] != NULL_VALUE_32)
							iter->GetTabPtr()->UnlockPackFromUse(iter->col_ndx, last_pack[cur_dim][threadt]);
					}

					for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++)
						last_pack[iter->dim][threadt] = NULL_VALUE_32;
					throw;
				}
			}
		}
		for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++)
			last_pack[iter->dim][threadt] = mit.GetCurPackrow(iter->dim);	// must be in a separate loop, otherwise for "a + b" will not lock b
	}
	else if(current_strategy == LOCK_ALL) {
		int fct=0;
		for(vector<VirtualColumn::VarMap>::const_iterator iter = my_vc.GetVarMap().begin(); iter != my_vc.GetVarMap().end(); iter++,fct++) {
			if(pack_locked[iter->dim][mit.GetCurPackrow(iter->dim)] == 0) {
				try {
					void *pf=NULL;
					uint range=mit.GetRangeInPack(iter->dim,mit.GetCurPackrow(iter->dim),&pf);
					iter->GetTabPtr()->LockPackForUse(iter->col_ndx,fsize>fct?prefetches[fct]:NULL, mit.GetCurPackrow(iter->dim), my_vc.ConnInfo(),pdesc,range,pf);
				} catch(...) {
					// unlock packs which are partially locked for this packrow
					vector<VirtualColumn::VarMap>::const_iterator it = my_vc.GetVarMap().begin();
					for(; it != iter; ++it) {
