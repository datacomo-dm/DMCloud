...
int OperationUnmysterify(Item* item, ColOperation& oper, bool& distinct, const int group_by_clause)
{
	...
	switch(static_cast<int>(item->type())) {
		...
		case Item::FIELD_ITEM:  // regular select
			oper = LISTING;/*ColOperation::GROUP_BY : ColOperation::LISTING;*/
			break;

		case Item::SUM_FUNC_ITEM: // min(k), max(k), count(), avg(k), sum
			switch (((Item_sum*)item)->sum_func()) {
				case Item_sum::MIN_FUNC:
					oper = MIN;/*ColOperation::MIN;*/
					break;
				case Item_sum::MAX_FUNC:
					oper = MAX;/*ColOperation::MAX;*/
					break;
				case Item_sum::SUM_DISTINCT_FUNC:
					distinct = true;
				case Item_sum::SUM_FUNC:
					oper = SUM; /*ColOperation::SUM;*/
					break;
				case Item_sum::AVG_DISTINCT_FUNC:
					distinct = true;
				case Item_sum::AVG_FUNC:
					oper = AVG;/*ColOperation::AVG;*/
					break;
				case Item_sum::COUNT_DISTINCT_FUNC:
					distinct = true;
				case Item_sum::COUNT_FUNC:
					oper = COUNT;/*ColOperation::COUNT;*/
					break;
				...
				case Item_sum::GROUP_CONCAT_FUNC:
					...
					// NOT IMPLEMENTED YET in the engine
					oper = GROUP_CONCAT;
					break;
				default:
					return RETURN_QUERY_TO_MYSQL_ROUTE;
