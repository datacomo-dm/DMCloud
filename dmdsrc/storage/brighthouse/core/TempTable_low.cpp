...
{	// "limit=10; offset=20" means that the first 10 positions of sorted table will contain objects 21...30.
	...
	for(int i = 0; i < ord.size(); i++)
		if(ord[i].vc != NULL) {
			ord[i].vc->InitPrefetching(it,ord.size());
			ord[i].vc->StopPrefetching();
			//ord[i].vc->Prefetch(it);
		}
	ord.clear();

	for(int i = 0; i< vc_for_prefetching.size(); i++) {
		vc_for_prefetching[i]->InitPrefetching(it,vc_for_prefetching.size());
		//vc_for_prefetching[i]->Prefetch(it);
	}
	...
	for(uint i = 0; i < NoAttrs(); i++) {
		if(attrs[i]->alias != NULL) {
			if(sender)
				attrs[i]->CreateBuffer(no_obj > RESULT_SENDER_CACHE_SIZE ? RESULT_SENDER_CACHE_SIZE : no_obj, &m_conn);
			else
...
void TempTable::FillMaterializedBuffers(_int64 local_limit, _int64 local_offset, ResultSender* sender, bool pagewise)
{
	...
	if(!filter.mind->ZeroTuples()) {
		...
		for(attrs_t::iterator attr = attrs.begin(), end = attrs.end(); attr != end; ++attr) /* fill buffers in attrs */
			if((*attr)->term.vc) {
				(*attr)->term.vc->InitPrefetching(it,attrs.size());
			}
...
std::vector<ATI> TempTable::GetATIs(bool orig)
{
	...
	for(uint i = 0; i < NoAttrs(); i++) {
		if(!IsDisplayAttr(i))
			continue;
		deas += ATI(attrs[i]->TypeName(), !attrs[i]->Type().GetNullsMode(), orig ? attrs[i]->orig_precision : attrs[i]->Type().GetPrecision(), attrs[i]->Type().GetScale(), false, false,attrs[i]->Type().IsPackIndex(),attrs[i]->Type().IsBloomFilter(),
			attrs[i]->Type().GetCompressType(), attrs[i]->Type().GetCollation());
	}
	return deas;
}
...
void TempTable::VerifyAttrsSizes()	// verifies attr[i].field_size basing on the current multiindex contents
{
	...
	Transaction *ptrans=ConnectionInfoOnTLS.Get().GetTransaction();
	for(uint i = 0; i < attrs.size(); i++) {
		if(!attrs[i]->term.vc) continue;
		SingleColumn* sc = (attrs[i]->term.vc->IsSingleColumn() ? static_cast<SingleColumn*>(attrs[i]->term.vc) : NULL);
		RCAttr* sec = NULL;
		if(sc)
			sec = dynamic_cast<RCAttr*>(sc->GetPhysical());
		if(sec && ATI::IsStringType(attrs[i]->TypeName()) && sec->GetPackInfoCollapsed())
			ptlist.LockOrCreate()->Start(boost::bind(&RCAttr::LoadPackInfo,sec,boost::ref(*ptrans)));
	}
	ptlist.WaitAllEnd();
	for(uint i = 0; i < attrs.size(); i++)
