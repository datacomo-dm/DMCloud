...
public:
	...
	int Compile(CompiledQuery* compiled_query, SELECT_LEX* selects_list, SELECT_LEX* last_distinct, TabID* res_tab = NULL, bool ignore_limit = false, Item* left_expr_for_subselect = NULL, Operator* oper_for_subselect = NULL, bool ignore_minmax = false);
...
private:
	...
	int VirtualColumnAlreadyExists(const TabID& tmp_table, const vector<int>& vcs, const AttrID& at);
...
public:
	...
	*/
	...
	static const std::string GetItemName(Item* item);
private:
...
//	WrapStatus::wrap_status_t WrapMysqlExpressionWithAggregations(Item *item, const TabID& tmp_table, MysqlExpression*& expr, bool* is_const_or_aggr = NULL);
	...
	int AddJoins(List<TABLE_LIST>& join, TabID& tmp_table, std::vector<TabID>& left_tables, std::vector<TabID>& right_tables, bool in_subquery, bool& first_table);
