...
FTree::FTree(const FTree &ft) :
	...
	value_size(ft.value_size), 	changed(ft.changed),
...
int FTree::ByteSize()
{
	if(changed==0 && compressed_size>0) return compressed_size;
	// header format: <format><dic_size><value_size><compressed_size><comp_mode><...len...>
	int header_size=12+2*dic_size;
	comp_mode=-1;
	if(dic_size==0) return header_size;

	int len_sum=0;
	for(int i=0;i<dic_size;i++) len_sum+=len[i];
...
void FTree::Init(unsigned char *&buf)
{
	...
	dic_size = *((int*) buf);
	buf += 4;
	total_dic_size = int((dic_size + 10) * 1.2);
	...
	for(i = 0; i < dic_size; i++) {
		len[i] = *((unsigned short int*) buf);
		value_offset[i] = (i == 0 ? 0 : value_offset[i - 1] + len[i - 1]);
		buf += 2;
	}
	for(i = dic_size; i < total_dic_size; i++) {
		len[i] = 0;
		value_offset[i] = NULL_VALUE_32;
	}
	...
	if(dic_size < 5)
		...
		total_buf_size = int((len_sum + 10) * 1.2);

	mem = (char*)alloc(total_buf_size * sizeof(char), BLOCK_TEMPORARY, true);
	if(!mem) {
		rclog << lock << "Error: FTree::Init, out of memory (" << total_dic_size * value_size << " bytes failed). (13)" << unlock;
		throw OutOfMemoryRCException();
	}
	memset(mem, 0, total_buf_size);
...
void FTree::InitHash()
{
	...
	while(hash_size % 2 == 0 || hash_size % 3 == 0 || hash_size % 5 == 0 || hash_size % 7 == 0)
		hash_size++;
	dealloc(hash_table);
	hash_table = (int *)alloc(hash_size * sizeof(int), BLOCK_TEMPORARY);
	if(hash_table == NULL)
		throw(OutOfMemoryRCException("Too many lookup values"));
...
size_t FTree::GetSizeInBytes()
{
	return rc_msize(mem) + rc_msize(len) + rc_msize(value_offset) + rc_msize(hash_table);
}
