...
protected:
	char	*GetRepresentation(int pack) { BHASSERT(pack <= end_pack, "Invalid pack request"); return (char *)(t.get() + (pack - start_pack) * no_positions  * 32 / sizeof(int)); }
...
private:
	...
	MMGuard<uint> t;		// buffer for values
};
