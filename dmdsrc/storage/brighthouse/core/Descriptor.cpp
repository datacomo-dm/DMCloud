...
right_dims(0), rv(RS_UNKNOWN), like_esc('\\'),distcode(0), desc_t(DT_NOT_KNOWN_YET), collation(DTCollation())
{
	op = O_UNKNOWN_FUNC;
}
...
Descriptor::Descriptor(TempTable* t, int no_dims)		// no_dims is a destination number of dimensions (multiindex may be smaller at this point)
	:	op(O_OR_TREE), lop(O_AND), sharp(false), encoded(false), done(false),
		...
		like_esc('\\'), distcode(0),desc_t(DT_NOT_KNOWN_YET), collation(DTCollation())
{ 
	assert(table);
}
...
Descriptor::Descriptor(CQTerm e1, Operator pr, CQTerm e2, CQTerm e3, TempTable* t, int no_dims, char like_escape) : lop(O_AND), sharp(false), 
	...
	right_dims(no_dims), rv(RS_UNKNOWN), desc_t(DT_NOT_KNOWN_YET), collation(DTCollation())
{
	op = pr;
	attr = e1;
	val1 = e2;
...
Descriptor::Descriptor(DescTree* sec_tree, TempTable* t, int no_dims) : op(O_OR_TREE), lop(O_AND), sharp(false), encoded(false), done(false), 
	...
	desc_t(DT_NOT_KNOWN_YET), collation(DTCollation())
{
	tree = NULL;
	distcode=0;
	if(sec_tree)
...
RSValue Descriptor::EvaluateRoughlyPackIndex(MIIterator& mit,std::vector<int> &packs)
{
	if(IsType_OrTree())
		return tree->root->EvaluateRoughlyPackIndex(mit,packs);
	RSValue r = RS_UNKNOWN;
	if(attr.vc && !attr.vc->IsConst()) {
		r = attr.vc->RoughPackIndexCheck(mit, *this,packs);
	}

	return r;
}

RSValue Descriptor::EvaluateRoughlyPack(MIIterator& mit)
{
	if(IsType_OrTree())
...
void Descriptor::Simplify(bool in_having)
{
	...
	switch(op) {
		...
		case O_NOT_BETWEEN: {
			...
			if(acc->IsNull(mit))
				res = false;
			else if(ATI::IsDateTimeType(acc->TypeName()) || ATI::IsDateTimeType(v1cc->TypeName()) || ATI::IsDateTimeType(v2cc->TypeName())) {
				RCDateTime x, y, z;

				AttributeType t = acc->TypeName();
				rc = acc->GetValueDateTime(mit, x, t);
				if(rc != BHRC_SUCCESS)
					throw(DataTypeConversionRCException("Conversion to numeric type failed"));
				rc = v1cc->GetValueDateTime(mit, y, t);
				if(rc != BHRC_SUCCESS)
					throw(DataTypeConversionRCException("Conversion to numeric type failed"));
				rc = v2cc->GetValueDateTime(mit, z, t);
				if(rc != BHRC_SUCCESS)
					throw(DataTypeConversionRCException("Conversion to numeric type failed"));
				res = RCDataType::compare(x, y, O_MORE_EQ, '\\') && RCDataType::compare(x, z, O_LESS_EQ, '\\');
			} else if(ATI::IsNumericType(acc->TypeName()) || ATI::IsNumericType(v1cc->TypeName()) || ATI::IsNumericType(v2cc->TypeName())) {
				RCNum x, y, z;

				rc = acc->GetValueRCNum(mit, x);
				if(rc != BHRC_SUCCESS)
					throw(DataTypeConversionRCException("Conversion to numeric type failed"));
				rc = v1cc->GetValueRCNum(mit, y);
				if(rc != BHRC_SUCCESS)
					throw(DataTypeConversionRCException("Conversion to numeric type failed"));
				rc = v2cc->GetValueRCNum(mit, z);
				if(rc != BHRC_SUCCESS)
					throw(DataTypeConversionRCException("Conversion to numeric type failed"));
				...
				BHASSERT(0, "Non comparable data types");
...
// get hash code of descriptor for identify pack got from riak node by condition
//return :
//  0 for unknow action
// otherwise for op|val hasecode to identify operation
// TODO: ÈçºÎ±ÜÃâ²éÑ¯Ìõ¼þµÄÉ¢ÁÐÖµÖØ¸´å?
// 31<---->24    <23-----0>
// 1xx ----     ---- ----
// 1111 ----     ---- ----  : between  OPER_NOT_BETWEEN
// 1110 ----     ---- ----  : between  OPER_BETWEEN
// 1101 ----     ---- ----  : in   OPER_NOT_IN
// 1100 ----     ---- ----  : in   OPER_IN
// 1011 ----     ---- ----  : like OPER_NOT_LIKE
// 1010 ----     ---- ----  : like OPER_LIKE
// 1000 ----     ---- ----  : other
#define  OPER_NOT_BETWEEN	0xf0000000
#define  OPER_BETWEEN		0xe0000000
#define  OPER_NOT_IN		0xd0000000
#define  OPER_IN			0xc0000000
#define  OPER_NOT_LIKE		0xb0000000
#define  OPER_LIKE			0xa0000000
//query with range combined with  list or rows-- merge with low 28 bits of transid
#define  OPER_HASHLIST		0x00000000
#define  OPER_SET(value,oper) (oper|(value&0x0fffffff))

uint Descriptor::GetHashCode() {
	if(tree || !attr.vc || attr.vc->IsConst()) return 0;
	if(distcode!=0) return distcode;
	distcode=0;
	if(RequiresUTFConversions(GetCollation())) return distcode;
	if(attr.vc->IsSingleColumn()!=VirtualColumnBase::SC_RCATTR) return distcode;
	MIIterator mit;// for dummy iterator
	RCAttr* a0 = (RCAttr *)((SingleColumn *)attr.vc)->GetPhysical();
	if((op == O_BETWEEN || op == O_NOT_BETWEEN) && val1.vc->IsConst() && val2.vc->IsConst()) {
		if(a0->PackType()==PackN) {
			_int64 pv1 = val1.vc->GetValueInt64(mit);
			_int64 pv2 = val2.vc->GetValueInt64(mit);
			distcode=leveldb::Hash((const char*)&pv1,sizeof(_int64),distcode);
			distcode=leveldb::Hash((const char*)&pv2,sizeof(_int64),distcode);
		}
		else {
			RCBString v1, v2;
			val1.vc->GetValueString(v1, mit);
			val2.vc->GetValueString(v2, mit);
			distcode=v1.GetHashCode()+v2.GetHashCode();
		}
		// make sure distcode is not zero;
		if (op == O_NOT_BETWEEN)
			distcode=OPER_SET(distcode,OPER_NOT_BETWEEN);
		else
			distcode=OPER_SET(distcode,OPER_BETWEEN);
	}
	else if((op == O_LIKE || op == O_NOT_LIKE) && val1.vc->IsConst() ) {
		RCBString v1;
		val1.vc->GetValueString(v1, mit);
		distcode=v1.GetHashCode();
		// make sure distcode is not zero;
		if (op == O_NOT_LIKE)
			distcode=OPER_SET(distcode,OPER_NOT_LIKE);
		else
			distcode=OPER_SET(distcode,OPER_LIKE);
	}
	else if((op == O_IN || op == O_NOT_IN) && val1.vc->IsInSet() && val1.vc->IsConst() ) {
		InSetColumn* isc = static_cast<InSetColumn*>(val1.vc);
		if( isc->NoValues(mit) > 0 && isc->NoValues(mit) < 64 ) {
			if(a0->PackType()==PackN) {
				size_t isc_size = isc->columns.size();
				for (size_t i=0; i<isc_size; i++) {
					int64_t pv1 = isc->columns[i]->GetValueInt64(mit);
					distcode=leveldb::Hash((const char*)&pv1,sizeof(int64_t),distcode);
				}
			} else {
				for(InSetColumn::Iterator it = isc->begin(mit), end = isc->end(mit); it != end; ++it) {
			...
			// make sure distcode is not zero;
			if (op == O_NOT_IN)
				distcode=OPER_SET(distcode,OPER_NOT_IN);
			else
				distcode=OPER_SET(distcode,OPER_IN);
		}
	}
	return distcode;
}

void Descriptor::StopPrefetching(bool unlock ) {
	if(tree)
		tree->root->StopPrefetching(unlock);
	if(attr.vc)
		attr.vc->StopPrefetching();
	if(val1.vc)
		val1.vc->StopPrefetching();
	if(val2.vc)
		val2.vc->StopPrefetching();
}


//set multi-thread pack preload in mit
void Descriptor::LockSourcePacks(const MIIterator& mit)
{
	if(tree)
		tree->root->PrepareToLock(0);
	if(attr.vc)
		attr.vc->LockSourcePacks(mit,this);
	if(val1.vc)
...
//with th_no: multi-thread pack preload disabled!
#ifdef __BH_COMMUNITY__
void Descriptor::LockSourcePacks(const MIIterator& mit, int th_no)
{
	if(tree)
		tree->root->PrepareToLock(th_no);// 0 --> th_no
	if(attr.vc)
		attr.vc->LockSourcePacks(mit,th_no);
	if(val1.vc)
		val1.vc->LockSourcePacks(mit,th_no);
	if(val2.vc)
		val2.vc->LockSourcePacks(mit,th_no);
}

void Descriptor::SetThreads(const MIIterator& mit , int threadn) {
	if(attr.vc)
		attr.vc->SetThreads(mit,threadn);
}

void Descriptor::EvaluatePack(MIUpdatingIterator& mit, _int64 *pahead)
{
	if(op==O_BETWEEN) {
	}
	// 
...
void Descriptor::EvaluatePack(MIUpdatingIterator& mit, int th_no)
{
	MEASURE_FET("Descriptor::EvaluatePack(...)");
	LockSourcePacks(mit,th_no);
    EvaluatePackImpl(mit);	
}
#endif
...
void Descriptor::UpdateVCStatistics()					// Apply all the information from constants etc. to involved VC
{
	...
	if(attr.vc->Type().IsNumeric() || (attr.vc->Type().IsLookup() && encoded)) {
...
BHTribool Descriptor::RoughCheckSubselectCondition(MIIterator& mit, SubSelectOptimizationType sot)
{
	...
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT(val1.vc && dynamic_cast<SubSelectColumn*>(val1.vc));
...
void Descriptor::CoerceColumnType(VirtualColumn*& for_typecast)
{
	...
	if(ATI::IsNumericType(vc->TypeName())) {
		if(ATI::IsTxtType(for_typecast->TypeName()))
			tcc = new String2IntCastColumn(for_typecast, vc->Type());
		else if(ATI::IsDateTimeType(for_typecast->TypeName()))
			tcc = new DateTime2IntCastColumn(for_typecast, vc->Type());
		...
		if(ATI::IsTxtType(for_typecast->TypeName()))
			...
			tcc = new Int2DateTimeCastColumn(for_typecast, vc->Type());
...
RSValue DescTreeNode::EvaluateRoughlyPack(MIIterator& mit)
{
	if(desc.op == O_OR_TREE) {
...
RSValue DescTreeNode::EvaluateRoughlyPackIndex(MIIterator& mit,std::vector<int> &packs)
{
	if(desc.op == O_OR_TREE) {
		std::vector<int> firstpacks;
		std::vector<int> secondpacks;

		RSValue left_res = left->EvaluateRoughlyPackIndex(mit,desc.lop == O_AND?firstpacks:packs);
		RSValue right_res = right->EvaluateRoughlyPackIndex(mit,desc.lop == O_AND?packs:secondpacks);
		
		if(left_res==RS_UNKNOWN || right_res==RS_UNKNOWN) {
			if(desc.lop == O_OR) return RS_UNKNOWN;
			if(left_res==RS_UNKNOWN && right_res==RS_UNKNOWN) return RS_UNKNOWN;
			if(left_res!=RS_UNKNOWN) // must be right_res is RS_UNKNOWN and left_res is not  RS_UNKNOWN and desc.lop is O_AND
				packs=firstpacks;
			return 	RS_SOME;
		}
		// any vector empty means result is empty on O_AND operation
		if(desc.lop == O_AND) { 
		   if(firstpacks.size()>0 && packs.size()>0)
		   {
			// merge using AND
			
			std::sort(firstpacks.begin(),firstpacks.end());
			std::sort(packs.begin(),packs.end());
			std::vector<int>::iterator iter=packs.begin();
			std::vector<int>::iterator iterfirst=firstpacks.begin();
			while(iter!=packs.end()) {
				while(*iter>*iterfirst &&iterfirst!=firstpacks.end()) {
					iterfirst++;
				}
				// remove all items which not contain in firstpacks,both packs are in order
				if(iterfirst==firstpacks.end() || *iter!=*iterfirst)
					iter=packs.erase(iter);
				else ++iter;
			}
		   }
		   else packs.clear();
		}
		else if(secondpacks.size()>0) { // O_OR
			//union packs
			if(packs.size()>0) {
				std::sort(secondpacks.begin(),secondpacks.end());
				std::sort(packs.begin(),packs.end());
				std::vector<int>::iterator iter=packs.begin();
				std::vector<int>::iterator itersecond=secondpacks.begin();
				while(itersecond!=secondpacks.end()) {
					if(iter==packs.end()) {
						packs.push_back(*itersecond);
						itersecond++;
					}
					else if(*itersecond>*iter) ++iter;
					else if(*itersecond==*iter) {
						++iter;
						++itersecond;
					}
					else {
	...
	return desc.EvaluateRoughlyPackIndex(mit,packs);
}
	...
	if(left) { // i.e., not a leaf
		...
		if(desc.lop == O_AND) {
			...
			if(left->desc.rv == RS_NONE)
				...
				int pack_start = mit.GetCurPackrow(single_dim);
				Filter *f = mit.NewPackFilter(pack_start);
				left->EvaluatePack(mit);
				if(mit.SwapPackFilter(pack_start, f)) {		// return true if the pack in the current iterator is not full
					mit.RewindToPack(pack_start);
					right->EvaluatePack(mit);
					mit.OrPackFilter(pack_start, f);
				}
				delete f;
			}
			return;
...
void DescTreeNode::StopPrefetching(bool unlock ) {
	if(left) { // i.e., not a leaf
		left->StopPrefetching(unlock);
		right->StopPrefetching(unlock);
	} else  { // i.e., a leaf
		locked = 0;
		desc.StopPrefetching(unlock);
	}
}
