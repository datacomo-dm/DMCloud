...
struct RSIndexID			// index identifier: index type plus index-dependent parameters (can be extended when needed)
{
	...
	int Load(FILE* f);
	int Save(FILE* f);
	std::string GetFileName(const char* const s);
...
{
public:
	RSI_Manager(const char *_path, int _index_l) throw(SystemRCException);
...
private:
	...
	static const size_t FILELEN = 128;
	char* path;				// copy of RSI Repository directory path (ended with '/', not necessarily null-termin.!)
	size_t pathlen;			// length of directory path in 'path'; the 'path' buffer can hold FILELEN more bytes to append file name
	...
	int SaveIndex(RSIndexID id, RSIndex* rsi, int write_loc);		// return 0 if OK
...
public:
	RSIndexPool(const char* _path);
	~RSIndexPool();

	int LoadDef(int table_id = -1);				// return value: positive for error, 0 when OK
	int SaveDef(int table_id = -1);
	int UpdateDef(int pos = -1);
	int UpdateDefForTable(int table_id = -1);
...
private:
	...
	char path[_PATHLEN];			// path to RSI definition file
...
public:
	...
	char buf[SZ];
	int Load(IBFile* frs_index,int current_loc)	{ buf[frs_index->Read(buf, SZ-1)] = 0; sscanf(buf, "%d", &time); return 0; }
	int LoadLastOnly(IBFile* frs_index,int current_loc)	{ buf[frs_index->ReadExact(buf, SZ-1)] = 0; sscanf(buf, "%d", &time); return 0; }
	int Save(IBFile* frs_index,int current_loc)	{ frs_index->Write(buf, sprintf(buf, "%d", time)); return 0; }
...
int	InitRSIManager(std::string datadir_path = "", bool with_updater = false, bool force = false);
