...
public:
	...
	~VCPackGuardian();
	void PreloadPack(const MIIterator &mit,std::vector<_int64 *> &prefetches);
	virtual PG_Status LockPackrow(const MIIterator &mit,std::vector<_int64 *> &prefetches,int threadt=0,Descriptor *pdesc=NULL);
	void UnlockAll();
	void WaitPreload();
	void SetThreads(const MIIterator &mit,int threadn) {
		Initialize(mit,threadn);
	}
protected:
