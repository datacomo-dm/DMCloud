...
//	virtual RCNum GetMinNum(int pack) const {BHASSERT(false, "not overridden"); return 0.0; };
	...
	*/
	...
	virtual RSValue RoughPackIndexCheck(MIUpdatingIterator &mit, int dim, Descriptor& d, bool additional_nulls_possible,std::vector<int> &packs)  {
		return RS_UNKNOWN;//sasu-277 sub query from temp table goes here,must return UNKNOWN
	}
