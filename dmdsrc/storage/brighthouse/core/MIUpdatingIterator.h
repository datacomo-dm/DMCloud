...
public:
	...
	MIUpdatingIterator():MIIterator(), changed(false), track_changes(false),
	...
	/*! Exclude the current packrow (defined by GetCurPackrow()) from the original multiindex---except the list rows.
	 *  Will be physically deleted not later than on Commit(), but usually immediately.
	 */
	void ResetCurrentPackBut(CRResultPtr &list);
