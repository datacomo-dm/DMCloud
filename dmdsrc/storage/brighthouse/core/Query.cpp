...
Query::Query(ConnectionInfo *conn_info)
	:	cq(NULL), t(NULL), no_tab(0), capacity(5), last_join(NULL), notifier(Notifier::E_Query, conn_info), m_conn(conn_info)
{
...
//		"XPATH_NODESET", "XPATH_NODESET_CMP", "VIEW_FIXER_ITEM" };
	...
	switch(static_cast<int>(item->type())) {
		...
		case Item_bhfield::BHFIELD_ITEM :
			...
			sprintf(buf, "BH_FIELD(T:%d,A:%d)", bh->varID.tab, bh->varID.col);
			return buf;
		}
	return "UNKNOWN";
#endif
		...
		int temp_table_alias, MultiIndex* mind)
{
	...
	if(exprs.size() == 1) {
		if(exprs[0]->IsDeterministic() && (exprs[0]->GetVars().size() == 0)) {
			...
			vc = new ConstColumn(exprs[0]->Evaluate(), type);
		} else if(VirtualColumn::IsConstExpression(exprs[0], temp_table_alias, &temp_table->GetAliases()) && exprs[0]->IsDeterministic()) {
			...
			if(static_cast<ExpressionColumn*>(vc)->GetStringType() ==  MysqlExpression::STRING_TIME) {
