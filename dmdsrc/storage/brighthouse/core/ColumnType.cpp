...
{
	if(dt.IsFixed()) {
		ColumnType ct(RC_NUM , AS_MISSED, false, QuickMath::precision10(dt.fixmax), dt.fixscale);
		std::swap(ct, *this);
	} else if(dt.IsFloat()) {
		ColumnType ct(RC_REAL, AS_MISSED, false, QuickMath::precision10(dt.fixmax), -1);
		std::swap(ct, *this);
	} else if(dt.IsString()) {
		ColumnType ct(dt.attrtype, AS_MISSED, false, dt.precision , 0, false,false,Compress_DEFAULT,dt.collation);
		std::swap(ct, *this);
	} else
