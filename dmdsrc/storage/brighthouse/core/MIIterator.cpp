...
uint MIIterator::GetRangeInPack(int dim,int p,void **pf) const
{	
	// for DummyIterator,it's not easy to map source iterator(may be not exists).
	if(it.size()<1 || !it_for_dim)
		return RANGEFULL;
	return it[it_for_dim[dim]]->GetRangeInPack(dim, p,pf);
}
