...
:	CacheableItem("JW", "INT"),lastpackpos(0), max_buffer_size_in_bytes(sec.max_buffer_size_in_bytes),
...
int IndexTable::GetNextPack(_uint64 cur_n,int ahead) {
		//TODO: refresh while set or swap on demond.
		if(packlist.size()==0 || int(cur_n >> block_shift) != cur_block) {
			// rebuild packlist;
			_uint64 end=size&block_mask;
			unsigned int old_packs=GetPackNo(0);
			packlist.clear();
			packlist.push_back(old_packs);
			for(_uint64 i=1;i<end;i++) {
				if(GetPackNo(i)!=old_packs) {
					old_packs=(GetPackNo(i));
					packlist.push_back(old_packs);
				}
			}
			lastpackpos=0;
		}
		int cpack=GetPackNo(cur_n);
		int psize=packlist.size();
		if(psize==0) return -1;
		if(packlist[lastpackpos]<cpack) { // seed forward
		  int i=lastpackpos+1;
		  for(;i<psize;i++)
		  {
		  	if(packlist[i]==cpack) break;
		  }
		  if(i==psize) return -1;
		  lastpackpos=i;
		}
		else if(packlist[lastpackpos]>cpack)  { // seek backward
		  int i=lastpackpos-1;
		  for(;i>-1;i--)
		  {
		  	if(packlist[i]==cpack) break;
		...
		if(lastpackpos+ahead>=psize) return -1;
		return packlist[lastpackpos+ahead];
	}