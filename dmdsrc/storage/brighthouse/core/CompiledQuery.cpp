...
void CompiledQuery::CQStep::Print()
{
	char buf[512] = "";
...
void CompiledQuery::TmpTable(TabID& t_out, const TabID& t1)
{
	CompiledQuery::CQStep s;
...
std::set<int> CompiledQuery::GetUsedDims(const TabID& table_id, std::vector<JustATablePtr>& ta)
{
	...
	for(int i = 0; i < NoSteps(); i++) {
		...
		} else if(!is_group_by && step.type == CompiledQuery::ADD_ORDER && step.t1 == table_id && step.a1.n != NULL_VALUE_32) {
...
bool CompiledQuery::IsGroupByQuery(const TabID& tab_id)
{
	...
	for(int i = 0; i < NoSteps(); i++) {
		step = Step(i);
		if(step.type == CompiledQuery::ADD_COLUMN && step.t1 == tab_id && step.cop == GROUP_BY)
			return true;
	}
	return false;
}
...
bool CompiledQuery::ExistsInTempTable(const TabID& tab_id, const TabID& tmp_table)
{
	...
	for(int i = 0; i < NoSteps(); i++) {
		step = Step(i);
		if(step.type == CompiledQuery::TMP_TABLE && step.t1 == tmp_table) {
