...
TempTable::Attr::Attr(CQTerm t, ColOperation m, bool dis, char* a, int dim, AttributeType type, uint scale, uint no_digits, NullMode n, DTCollation collation)
	: mode(m), distinct(dis), term(t), dim(dim)
{
	ct.Initialize(type, n, false, no_digits, scale, false,false,Compress_DEFAULT,collation);
...
void TempTable::Attr::CreateBuffer(_uint64 size, ConnectionInfo *conn)
{
	...
	switch(TypeName()) {
		...
		case RC_BYTE:
		case RC_VARBYTE:
			buffer = new AttrBuffer<RCBString> (page_size, this->Type().GetInternalSize() + 4, conn);
			break;
		case RC_BIGINT:
		case RC_NUM:
		case RC_YEAR:
...
bool TempTable::Attr::IsNull(const _int64 obj) const
{
	...
	switch(TypeName()) {
		...
		case RC_REAL:
		case RC_FLOAT:
			res = (*(AttrBuffer<double> *) buffer)[obj] == NULL_VALUE_D;
			break;
	}
	return res;
}
...
TempTable::TempTable(const TempTable& t, bool subq, bool _for_rough_query)
	: filter(t.filter, _for_rough_query), output_mind(t.output_mind), m_conn(t.m_conn)
{
	for_rough_query = _for_rough_query;
	no_obj = t.no_obj;
	materialized = subq ? false : t.materialized;
	aliases = t.aliases;
	group_by = t.group_by;
	no_cols = t.no_cols;
	mode = t.mode;
	...
	if(for_subq) {
		filter.table = this;
		for(uint i = 0; i < filter.descriptors.Size(); i++)
			filter.descriptors[i].table = this;
		for(uint i = 0; i < filter.parametrized_desc.Size(); i++)
			filter.parametrized_desc[i].table = this;
	}
	for(uint i = 0; i< t.attrs.size(); i++) {
		if(for_rough_query)
			attrs.push_back(new Attr(*t.attrs[i]));
		else if(subq) {
			//t.attrs[i]->DeleteBuffer();
			//assert(t.attrs[i]->buffer == NULL);
...
TempTablePtr TempTable::CreateMaterializedCopy(bool translate_order)			// move all buffers to a newly created TempTable, make this VC-pointing to it; the new table must be deleted by DeleteMaterializedCopy()
{
	...
	TempTablePtr working_copy = Create(*this, false);		// Original VCs of this will be copied to working_copy, and then deleted in its destructor
	for(uint i = 0; i < attrs.size(); i++) {
	...
	} else {	// Create() did moved vc's used in order_by into working_copy, so we should bring them back
		for(uint i = 0; i < order_by.size(); i++) {
			// find which working_copy->vc is used in ordering
			uint j = 0;
			for(j = 0; j < working_copy->virt_cols.size(); j++) {
				if(working_copy->order_by[i].vc == working_copy->virt_cols[j])
					break;
			}
			if(j < working_copy->virt_cols.size()) {
				virt_cols.push_back(working_copy->virt_cols[j]);
				order_by[i].vc = working_copy->virt_cols[j];
				working_copy->virt_cols[j] = NULL;				// moving vc - now it is back in this
			}
		}
	}
...
int TempTable::AddColumn(CQTerm e, ColOperation mode, char * alias, bool distinct)
{
	...
	if (e.vc_id != NULL_VALUE_32) {
		...
		if(mode == COUNT || mode == BIT_AND || mode == BIT_OR || mode == BIT_XOR) {
...
//						throw NotImplementedRCException("SUM performed on non-numerical column.");
	...
	attrs.push_back(new Attr(e, mode, distinct, alias, -2, type, scale, precision, is_nullable, e.vc ? e.vc->GetCollation() : DTCollation()));
...
void TempTable::Union(TempTable* t, int all)
{
	...
	::Filter first_f(NoObj(),nfblocks), first_mask(NoObj(),nfblocks); // mask of objects to be added to the final result set
	::Filter sec_f(t->NoObj(),nfblocks), sec_mask(t->NoObj(),nfblocks);
	first_mask.Set();
	sec_mask.Set();
	if(!all && NoObj() && t->NoObj()) {
		rccontrol.lock(m_conn.GetThreadID()) << "UNION: excluding repetitions." << unlock;
		::Filter first_f(NoObj(),nfblocks);
		first_f.Set();
		::Filter sec_f(t->NoObj(),nfblocks);
		sec_f.Set();
		...
		{	// block to ensure deleting encoder before deleting first_vcs, sec_vcs
			...
			for(int i = 0; i < (int)NoDisplaybleAttrs(); i++) {
				...
				if(encoder[i].PrepareEncoder(first_vcs[i].get(), sec_vcs[i].get()) == false) {
					stringstream ss;
			...
			dist_table.InitializeB(size, NoObj());
...
void TempTable::ProcessParameters()
{
	MEASURE_FET("TempTable::ProcessParameters(...)");
	for(uint i = 0; i< virt_cols.size(); i++)
		virt_cols[i]->RequestEval();
...
void TempTable::RoughProcessParameters()
{
...
TempTablePtr TempTable::Create(const TempTable& t, bool for_subq, bool for_rough_query)
{
	MEASURE_FET("TempTable::Create(...)");
	TempTablePtr tnew = boost::shared_ptr<TempTable>(new TempTable(t, for_subq, for_rough_query));
	if(for_subq) {
...
TempTablePtr TempTable::Create(JustATable* const t, int alias, Query* q)
{
...
ColumnType TempTable::GetUnionType(ColumnType type1, ColumnType type2)
{
	...
	if(type2.IsString() && type2.GetPrecision() != 0 && !type1.IsString())
		return type2;
	if(type1.GetTypeName() == RC_NUM && type2.GetTypeName() == RC_NUM && type1.GetScale() != type2.GetScale()) {
...
void TempTable::LockPackForUse(unsigned attr, _int64 *pahead, unsigned pack_no, ConnectionInfo& conn,Descriptor *pdesc,uint range,void *pf)
{
	while(lazy && no_materialized < min(((_int64)pack_no << 16) + 0x10000, no_obj))
		Materialize(false, NULL, true);
}

void TempTable::PreloadPack(unsigned attr, _int64 *pahead,ConnectionInfo& conn,Descriptor *pdesc)
{}

void TempTable::WaitPreload(unsigned attr,ulong transid)
{}
...
void TempTable::Materialize(bool in_subq, ResultSender* sender, bool lazy)
{
	...
	if(materialized && (order_by.size() > 0 || limits_present) && no_obj) {
		...
		if(order_by.size() != 0 && no_obj > 1) {
			TempTablePtr temporary_source_table = CreateMaterializedCopy(true);		// true: translate definition of ordering
	...
	if(distinct_on_materialized && no_obj > 1) {
		...
		if(exists_only)
			local_limit = 1;
		TempTablePtr temporary_source_table = CreateMaterializedCopy(false);
...
void TempTable::RecordIterator::PrepareValues()
{
	if (_currentRNo < _uint64(table->NoObj())) {
		...
		for(int att = 0; att < no_disp_attr; ++att) {
...
: table( it.table ), _currentRNo( it._currentRNo ), _conn( it._conn )
{
