...
{
	static size_t calc( T const* v )
		{ return ( ( v[0] << 8 ) ^ ( v[1] << 6 ) ^ ( v[2] << 4 ) ^ ( v[3] << 2 ) ^ v[4] ); }
};
...
public:
	...
	bool operator == ( ObjectId const& oid ) const {
		return ( object_id_helper::range_eq<T const*>()( _coord, _coord + no_dims, oid._coord ) );
	}
	bool operator < ( ObjectId const& oid ) const {
		return ( object_id_helper::range_less<T const*>()( _coord, _coord +  no_dims, oid._coord ) );
	}
	size_t hash( void ) const {
		return ( object_id_helper::hasher<T, no_dims>::calc( _coord ) );
	}
	bool operator() ( ObjectId const& oid1, ObjectId const& oid2 ) const {
		return ( cmp_algo<T const*>()( oid1._coord, oid1._coord + no_dims, oid2._coord ) );
	}
...
class TOCoordinate : public BucketInfo {
	...
	TOCoordinate( const TOCoordinate &_t ) : ID(_t.ID) { co.pack = _t.co.pack; co.rcattr = _t.co.rcattr; }
