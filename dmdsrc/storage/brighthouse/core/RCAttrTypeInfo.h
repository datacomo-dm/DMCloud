...
public:
	AttributeTypeInfo(AttributeType attrt, bool notnull, ushort precision = 0, ushort scale = 0, bool lookup = false, 
		bool for_insert_ = false, bool packindex=false,bool bloomfilter=false,int _compress_type=Compress_DEFAULT,DTCollation collation = DTCollation())
		: attrt(attrt), precision(precision), scale(scale), notnull(notnull), lookup(lookup), for_insert(for_insert_),packindex(packindex),bloomfilter(bloomfilter),
		compress_type(_compress_type),parameter(0), collation(collation), decomposition()
	{
	}

	uint	ExtrnalSize(EDF edf);
	int		TextSize();
	...
	bool PackIndex() const			{ return packindex; }
	bool BloomFilter() const			{ return bloomfilter; }
	uint Params() const
	{
		uint parameter = 0;
		if(lookup)
		...
		if(compress_type==Compress_DEFAULT)
			parameter |= 0x00000020;
		if(compress_type==Compress_Soft_Zip || compress_type == Compress_Hard_Zip)
	...
	inline	void 	SetCompressType(const int comType){compress_type = comType;}
