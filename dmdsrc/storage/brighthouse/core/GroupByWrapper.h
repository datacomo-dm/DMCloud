...
public:
	...
	void InitTuples(_int64 _no_obj);
...
public:
	...
	bool PackOutsideDistinctBuffer(int i, int pack);		// omit pack, because values out of scope of the current distinct buffer
	...
	void DistinctlyOmitted(int attr, _int64 obj/*, _int64 pos*/);
	...
	void Initialize(_int64 upper_approx_of_groups);
	...
	int GetOccupiedRow(int n)		{ return gt->GetOccupiedRow(n); }
	int GetNoOfOccupiedRows()		{ return gt->GetNoOfOccupiedRows(); }
	_int64 GetValue64(int col, _int64 row)	{ return gt->GetValue64(col, row);	}

	RCBString GetValueT(int col, _int64 row);

	void ClearAll()								{ gt->ClearAll(); }
	void ClearUsed()							{ gt->ClearUsed(); }
	void ClearDistinctBuffers();				// reset distonct buffers and distinct cache
	void RewindDistinctBuffers();				// reset distinct buffers, rewind distinct cache to use it contents
	void SetDistinctTuples(_int64 no_tuples)	{ distinct_watch.InitTuples(no_tuples); }
	...
	int MaxNoAggregators()						{ return gt->MaxNoAggregators(); }		// how many independent aggregators may fit in memory
