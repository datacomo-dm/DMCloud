...
int RCEngine::Init(const char* tab_ext, uint engine_slot)
{
    ...
    if(! DoesFileExist(infobright_data_dir + "brighthouse.seq")) {
		m_tab_sequencer = 0;
    ...
    //>> begin: fix dma-1105 add by liujs 
    //   clear_loading_file();
    thd_clear_loading.LockOrCreate()->Start(boost::bind(&RCEngine::clear_loading_file,this)); 
    //<< end:  fix dma-1105 add by liujs 
    ...
    if(InitRSIManager(infobright_data_dir, true) != 0)
    	return 1;
...
RCEngine::~RCEngine()
{
    thd_clear_loading.WaitAllEnd();
    
	MultiThread_Preload::ReleaseInstance();
	rsi_manager.reset();
...
*****************************************************/
	...
	GlobalDataCache::GetGlobalDataCache().ReleaseAll(true);
	...
	AttrIndex::release_leveldb_index();
...
BHErrorCode RCEngine::CreateTable(const char *table, TABLE *from)
{
	...
	dm_strlcpy(tab_path, table, path_len);
	dm_strlcat(tab_path, m_tab_ext, path_len);
    ...
    UnsupportedDataTypeRCException msg_blob("Error: RCEngine::CreateTable not support MYSQL_TYPE_BLOB type.");
    UnsupportedDataTypeRCException msg_other("Error: RCEngine::CreateTable not support default type.");
		...
		while(field != 0) {
             switch(field->type()){
				...
				case MYSQL_TYPE_BLOB: 
					rclog << lock << "Error: RCEngine::CreateTable not support MYSQL_TYPE_BLOB type."<< unlock;
                    throw msg_blob;
					break;
				default:
					rclog << lock << "Error: RCEngine::CreateTable not support default type."<< unlock;
                    throw msg_other;
                    break;
            }
    ...
    try{
        ...
        rclog << lock << "Error: RCEngine::CreateTable error,information in brighthouse.log."<< unlock;
        DeleteDirectory(tab->GetPath());
        return BHERROR_UNKNOWN;
    }catch(...){
        rclog << lock << "Error: RCEngine::CreateTable error,information in brighthouse.log."<< unlock;
        DeleteDirectory(tab->GetPath());
...
//		is_lookup = true;
	...
	std::transform(c.begin(),c.end(),c.begin(),easytolower);
	
	if(int(c.find("lookup"))!= string::npos)
		is_lookup = true;
	
	if(int(c.find("for_insert")) != string::npos)
		for_insert = true;
	
	bool is_packindex = false;
	if(int(c.find("packindex")) != string::npos)
		is_packindex = true;

	int compress_type = Compress_DEFAULT;
	if(int(c.find("zipcompression")) != std::string::npos)
		compress_type = Compress_Soft_Zip;
	
	bool is_bloomfilter = false;
	if(int(c.find("bloomfilter")) != string::npos)
		is_bloomfilter = true;
	...
	switch(field.type()) {
		...
		case MYSQL_TYPE_NEWDATE:
			return ATI(RCEngine::GetCorrespondingType(field), field.null_bit == 0, (ushort)field.field_length, 0,
					is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type);
		case MYSQL_TYPE_TIME:
			return ATI(RC_TIME, field.null_bit == 0, 0, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type);
			/*case MYSQL_TYPE_YEAR :
		...
		case MYSQL_TYPE_VARCHAR: {
			...
			if(const Field_str* fstr = dynamic_cast<const Field_string*>(&field))
			{
				if(fstr->charset() != &my_charset_bin)
					return ATI(RC_STRING, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type, DTCollation(fstr->charset(), fstr->derivation()));
				return ATI(RC_BYTE, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type,DTCollation(fstr->charset(), fstr->derivation()));
			}
			else if(const Field_str* fvstr = dynamic_cast<const Field_varstring*>(&field))
			{
				if(fvstr->charset() != &my_charset_bin)
					return ATI(RC_VARCHAR, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type, DTCollation(fvstr->charset(), fvstr->derivation()));
				return ATI(RC_VARBYTE, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type, DTCollation(fvstr->charset(), fvstr->derivation()));
		...
		case MYSQL_TYPE_NEWDECIMAL :
		{
			...
			return ATI(RC_NUM, field.null_bit == 0, fnd->precision, fnd->dec, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type);
			throw UnsupportedDataTypeRCException();
		}
		case MYSQL_TYPE_BLOB :
		if(const Field_str* fstr = dynamic_cast<const Field_str*>(&field))
		{
			if(const Field_blob* fblo = dynamic_cast<const Field_blob*>(fstr))
			{
				...
				{//TINYTEXT, MEDIUMTEXT, TEXT, LONGTEXT
					if(field.field_length> 65535)
					throw UnsupportedDataTypeRCException();
					return ATI(RC_VARCHAR, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type, DTCollation());
				}
				else
				{
					switch(field.field_length)
					{
						...
						case 65535 :
						//TINYBLOB, BLOB
						return ATI(RC_VARBYTE, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type, DTCollation());
						case 16777215 :
						case 4294967295 :
						//MEDIUMBLOB, LONGBLOB
						return ATI(RC_BIN, field.null_bit == 0, (ushort)field.field_length, 0, is_lookup, for_insert,is_packindex,is_bloomfilter,compress_type);
						default:
						throw UnsupportedDataTypeRCException();
					}
				}
...
int RCEngine::Commit(THD* thd, bool all)
{
#ifdef PURE_LIBRARY
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
	return 0;
...
ConnectionInfo* RCEngine::GetThreadParams(const THD& thd)
{
	...
	if(thd.ha_data[m_engine_slot].ha_ptr == NULL)
		CreateThreadParams(const_cast<THD&>(thd));
	return (ConnectionInfo*)thd.ha_data[m_engine_slot].ha_ptr;
#endif
}

void RCEngine::CreateThreadParams(THD& thd)
{
#ifdef PURE_LIBRARY
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
#else
...
//>> begin: fix dma-1180
void RCEngine::clear_dpadmin_lst_file(){
    char dirname[256];
    const char *filetype=".lst";
    strcpy(dirname,infobright_data_dir.c_str());
    std::vector<std::string> dpadmin_list;
    get_type_file(dirname,filetype,dpadmin_list,false);

    for(int i=0;i<dpadmin_list.size();i++){//dpadmin_%s.%s_sz_%d.lst
        std::string _dpadmin_file = dpadmin_list[i];
        char tmp[256] ;
        strcpy(tmp,_dpadmin_file.c_str());
        char* p = tmp+strlen(tmp)-1;
        while(*p != '/' && *p) p--;       
        if(*p){
            if(strncmp(p+1,"dpadmin_",8) == 0){
                rclog << lock << "Remove dpadmin dload list file [ " << _dpadmin_file.c_str()<< "]." << unlock;
                RemoveFile(_dpadmin_file);
            }
        }
    }
    dpadmin_list.clear();
...
//>> begin: fix dma-1105 add by liujs 
void RCEngine::clear_loading_file(){
    char dirname[256];
    const char *filetype=".loading";
    strcpy(dirname,infobright_data_dir.c_str());
    std::vector<std::string> loading_list;
    get_type_file(dirname,filetype,loading_list,true);

    for(int i=0;i<loading_list.size();i++){
        std::string _loading_file = loading_list[i];
        rclog << lock << "Remove loading file [ " << _loading_file.c_str()<< "]." << unlock;
        RemoveFile(_loading_file);
    }
...
void RCEngine::get_type_file(const char* dirpath,const char* filetype, std::vector<std::string>& loading_list,bool recursion){
    DIR *dir_ptr;          
    ...
    if (( dir_ptr = opendir(dirpath))== NULL){
        ...
        while ((direntp = readdir(dir_ptr )) != NULL){
            ...
            if (ptr1==0 || ptr2==0){
                ...
                const char* ch = "/";   
                int i=0;
                strcpy(filefullpath, dirpath);   
                strcat(filefullpath, ch);   
                assert(strlen(direntp->d_name)<255);
                strcat(filefullpath, direntp->d_name); 
                assert(strlen(filefullpath)<255);
                if(is_type_file(direntp->d_name,filetype)==1){
                    std::string _full_name(filefullpath);
                    loading_list.push_back(_full_name);
                }
                if(recursion){
                    get_type_file(filefullpath,filetype,loading_list,recursion);   
                }
            }     
        }   
        closedir(dir_ptr);   
...
// 判断文件filepath是否是filetype类型的
int  RCEngine::is_type_file(const char* filepath,const char* filetype){
    int i;
    int len=strlen(filepath)/sizeof(char);
    int typelen=strlen(filetype)/sizeof(char);
    char ext[256];
    strncpy(ext,&filepath[len-typelen],(typelen>256)?255:typelen);
...
int get_parameter(THD* thd, enum bh_var_name vn, longlong &result, string& s_result)
{
	...
	const uint var_size = 1024;
	char var_data[var_size];

	switch(vn) {
		case BH_TIMEOUT:
			strcpy(var_data, "BH_TIMEOUT");
			break;
		case BH_DATAFORMAT:
			strcpy(var_data, "BH_DATAFORMAT");
			break;
		case BH_PIPEMODE:
			strcpy(var_data, "BH_PIPEMODE");
			break;
		case BH_NULL:
			strcpy(var_data, "BH_NULL");
			break;
		case BH_THROTTLE:
			strcpy(var_data, "BH_THROTTLE");
			break;
		default:
			return 1;
	}

	if(m_entry = (user_var_entry*) hash_search(&thd->user_vars, (uchar*) var_data, (uint)strlen(var_data))) {
		if(m_entry->type == INT_RESULT) {
			if(vn != BH_TIMEOUT && vn != BH_THROTTLE) {
				result = -1;
				return 0;
			} else {
			...
			if(m_entry->value == NULL)
				return 1;
			dm_strlcpy(var_data, m_entry->value, sizeof(var_data));
			if(vn == BH_DATAFORMAT) {
				s_result = var_data;
			} else if(vn == BH_PIPEMODE) {
				if(stricmp(var_data, "SERVER") == 0)
					result = 1;
				if(stricmp(var_data, "CLIENT") == 0)
					result = 0;
			} else if(vn == BH_NULL) {
				s_result = var_data;
			}
