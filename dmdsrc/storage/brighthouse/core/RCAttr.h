...
#include "attrindex.h"
#include "partinfo.h"
#include "core/DMThreadGroup.h"
#include "core/MapPackInfo.h"
...
class RCAttr: public TrackableObject, public PhysicalColumn {
	...
	std::map<ulong,DMThreadData *> running_ThreadData;
public:
	//TODO: Need more refactoring to have functions like Load/save(tablename, attrname);
	bool	Load(std::string const& cmd_file,bool loadapindex=true);
	bool	Save(std::string const& cmd_file, FTree* dic, _int64 comprs_size);
  // Get not full (65536) blocks,filled at constructor and updated at loading.
  std::map<int,int> *GetNFBlocks() {return &nfblocks;}
  // truncate a partition
  //  the partition be truncated is trans by a control file
    bool TruncatePartition(int connectid) ;
    void ClearPartitionCtrlFile(const long con_id = 0) {
        if(partitioninfo!=NULL){ 
            partitioninfo->CleanCtrFile(con_id);
        }
    }

	bool DoDrop();
    bool CommitTruncatePartiton();
	bool RollbackTruncatePartition();
	void RemoveTruncateControlFile();
...
// Methods declared in PhysicalColumn.h or RCAttr-specific
	...
	RCAttr(int a_num, int t_num, std::string const& a_path, int conn_mode = 0, unsigned int s_id = 0,bool loadapindex=true, DTCollation collation = DTCollation())
	...
	virtual inline _int64 GetValueInt64(_int64 obj) const
	{
		...
		//obj composed by pack and offset,may be exceed no_obj after partition support
		//assert(no_obj >= obj);
		...
		if(dpn.is_stored) {
			...
			if(dpn.IsNull(inpack))
				return NULL_VALUE_64;
			_int64 res = dpn.GetPackN()->GetVal64(inpack);	// 2-level encoding
			// Natural encoding
		...
		assert(0);		// neither loaded nor uniform/null only
		return NULL_VALUE_64;
	}
	...
	virtual inline _int64 GetNotNullValueInt64(_int64 obj) const
	{
		...
		if(dpn.is_stored) {
			...
			_int64 res = dpn.GetPackN()->GetVal64((int)(obj&65535));	// 2-level encoding
			// Natural encoding
			if(ATI::IsRealType(TypeName()))
				return res;
			res += dpn.local_min;
	...
	inline bool IsNull(_int64 obj) const
	{
		...
		if (dpn.is_stored) {
			...
			return dpn.IsNull((int)(obj&65535));				// return 1 if the value of attr. is null
		}
	...
	int Collapse();	
	const char *MergeIndexName(char *name);
	virtual void	LockPackForUse(_int64 *pahead,unsigned pack_no,ConnectionInfo& conn,Descriptor *pdesc, uint range=RANGEFULL,void *pf=NULL);
	void LockPackForUse(_int64 *pahead,unsigned pack_no,Transaction& trans,Descriptor *pdesc, boost::function0<void> notify_pack_load = boost::function0<void>(), uint range=RANGEFULL,void *pf=NULL);
	virtual void	LockPackForUse(unsigned pack_no);
	int LockPackForThread(unsigned pack_no, Transaction& trans,uint range,Descriptor *pdesc,uint dkey,void *pf);
	void DeliverPreLoad(_int64 *pahead,DMThreadGroup *pmtr,Transaction& trans,uint chash,Descriptor *pdesc,DMThreadData *pThreadData);
	void PreLoadPack(_int64 *pahead,ConnectionInfo& conn,Descriptor *pdesc=NULL);
	virtual void	UnlockPackFromUse(unsigned pack_no);
	void WaitPreload(ulong transid);
...
//	int ColumnSize()	 const;
	...
	void DeleteRSIs();
	//merge hash only,not write back to old db or new db
	void MergeIndexHash(DMThreadGroup *ptlist,uint s_id);
	void DoMergeIndexHash(DMThreadData *tl,uint s_id);
	void  ResetRiakPNode();
	//merge hash , write back to new db
	void MergeIndex(DMThreadGroup *ptlist,uint s_id);
	void DoMergeIndex(DMThreadData *tl,uint s_id);
	bool IsPackIndex();
	...
	int PartLastPackObjs() {
		return partitioninfo->getsavepartinfo(NULL).getlastpackobjs();
	}
	bool IsLastLoad() {
		return partitioninfo->IsLastLoad();
	}
	int PartLastSavePackObjs() {
		return partitioninfo->getsavepartinfo(NULL).getsaveobjs();
	}
	void ClearMergeDB() {
		if(ct.IsPackIndex()) {
			char mergedbname[300];
			std::string idxpath=path+"/"+MergeIndexName(mergedbname);
			DeleteDirectory( idxpath);
		}
	}
	const char *GetLoadPartName() ;
	...
	int				DecodeValue_T(_int64 code,char *val_buf);											// original 0-level text value for a given 1-level code; return the number of characters
	...
	RSValue RoughPackIndexCheck(MIUpdatingIterator &mit, int dim, Descriptor& d, bool additional_nulls_possible,std::vector<int> &packs);	// check whether any value from the pack may meet the condition
...
//	bool VerifyRSI(); 					// Repairs index directory: false if the repair was needed and done.
	...
	//static IBMutex m_preload_mutex;
	//std::vector<ushort> packs_locks;
	// Session and saving information
	int current_state;					// 0 - opened as read only, 1 - writing session
	...
	std::string path;
	int last_read_packs,cont_read_times;
    
	// Rough set indexes
	RSIndex_Hist* rsi_hist_update;
	RSIndex_CMap* rsi_cmap_update;
	AttrIndex *ldb_index;// level db index entry,for both update & query
	attr_partitions *partitioninfo;// partition support for loading & query
	int file;
...
protected:
	...
	inline std::string GetRiakKey(const int64 pack_no) {
		return _get_riak_key(this->table_number, this->attr_number,pack_no);
	}
...
private:
	...
	inline _int64 GetTable64Loaded( uint pack, int n )	// low-level, fast: no pack loading etc.
	{
		...
		if( dpns[pack].GetPackN()->IsNull(n) )
			return NULL_VALUE_64;
		_int64 res = ((AttrPackN*)dpns[pack].GetPackN())->GetVal64( n );	// 2-level encoding
		// Natural encoding
		if (ATI::IsRealType(TypeName()))
			return res;
...
#if PERFORMANCE_SASU_54     // 测试性能的开关
	mytimer   _my_timer_save_pack;    // 单个包保存时间
	mytimer   _my_timer_save_pack_sum; // 一次装入的包的保存时间和
	mytimer	  _my_timer_comp_pack_sum; // 一次装入的包的压缩时间和
	mytimer	  _my_timer_rsi_pack_sum; // 一次装入rsi的时间
	void display_save_pack_time(){
		DPRINTF("----> save pack_sum<%d,%d> use time %0.4f（s）.\n",table_number,attr_number,_my_timer_save_pack_sum.GetTime());
		DPRINTF("----> cmpr pack_sum<%d,%d> use time %0.4f（s）.\n",table_number,attr_number,_my_timer_comp_pack_sum.GetTime());
		DPRINTF("----> rsi pack_sum<%d,%d> use time %0.4f（s）.\n",table_number,attr_number,_my_timer_rsi_pack_sum.GetTime());
	}
#endif
};
...
AttrPackPtr AttrPackLoad(Descriptor *pdesc,const PackCoordinate& coord_, PackAllocator &a,void *pf);
...
extern "C" double getfreecpu(int &cpuct);
class MultiThread_Preload {
public:
	struct push_param {
		_int64 pahead[PREFETCH_BUFL];
		uint chash;
		RCAttr *pattr;
		Transaction* trans;
		Descriptor *pdesc;	
		push_param(_int64 *_pahead,int _chash,RCAttr *_pattr,Transaction *_trans,Descriptor *_pdesc):
			chash(_chash),pattr(_pattr),trans(_trans),pdesc(_pdesc) {
				memcpy(pahead,_pahead,PREFETCH_BUFL*sizeof(_int64));
		}
	};
	
private:
	static MultiThread_Preload *instance;
	int max_mtr;
	DMThreadGroup *pusher;
	DMThreadGroup *pmtr;
	MultiThread_Preload() {
		max_mtr=60;
		pmtr=new DMThreadGroup(max_mtr);
		pusher=new DMThreadGroup(5,60);
	}
public :
	~MultiThread_Preload() {
		delete pmtr;
	...
	static MultiThread_Preload *GetInstance() {
		if(instance!=NULL) return instance;
		// prevent concurrentcy:
		static DMMutex init_lock;
		init_lock.Lock();
		if(instance==NULL) {
			instance=new MultiThread_Preload();
		}
		init_lock.Unlock();
		return instance;
	}
	static void PushThread(DMThreadData *pdata) {
		push_param *opts=(push_param *)pdata->StructParam();
		opts->pattr->DeliverPreLoad(opts->pahead, GetInstance()->pmtr,*opts->trans,opts->chash,opts->pdesc,pdata);
	}
	// push to thread list or push to buffer(async?)
	// return 0: empty preload data 1:has pushed preload data
	int PushPreload(_int64 *pahead,uint chash,RCAttr *pattr,Transaction& trans,Descriptor *pdesc) {
		if(pahead[0]-pahead[1]<1) return 0;
		//push 数据队列满,则放弃预读取
		if(pusher->IsIdleEmpty()) {
			//partial droping preload 10%
			int droped=(pahead[0]-pahead[1])*0.1;
			if(droped<1) droped=1;
			pahead[1]+=droped;
			static long dropped_pushqueue=0;
			dropped_pushqueue++;
			if(rccontrol.isOn() && RIAK_OPER_LOG>=1) {
				rclog << lock << "Push queue dropped  "<<dropped_pushqueue<<"."<<unlock;
			}
			return 0;
		}		
		push_param opts(pahead,chash,pattr,&trans,pdesc);
		DMThreadData *ptd=pusher->WaitIdleAndLock();
		ptd->Start((const char *) (&opts), sizeof(push_param), PushThread);
		//clear preload
		pahead[1]=pahead[0];
		return 1;
	}
	...
	void StopPrefetching(RCAttr *pattr,ulong sid) ;
};
