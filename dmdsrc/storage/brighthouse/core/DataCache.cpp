...
void DataCache::ReleaseAll(bool killed)
{
	if(m_local) {
	...
	_ftrees.clear();
	_filters.clear();
	//sasu-264:remove list item from FreeList[49] very slowly
	// clear TCMHeap Freelist Item before clear objects
	if(killed) {
		IBGuard m_cache_guard(m_cache_mutex);
		SliceContainer::iterator it = _slices.begin();
		if(it != _slices.end()) {
			it->second->clear_freelists();
		}
	}
	_slices.clear();
}
