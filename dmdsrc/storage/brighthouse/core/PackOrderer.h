...
//	PackOrderer(std::vector<VirtualColumn*> vcs, std::vector<OrderType> orders, RSValue* r_filter);
	...
	bool Init(std::vector<VirtualColumn*> vcs, std::vector<OrderType> orders, int no_unsorted_cols = 0, RSValue* r_filter = NULL);
	...
	*/
	 ...
	 */
	static void ChoosePackOrderer(PackOrderer& po, const std::vector<OrderingInfo>&);
