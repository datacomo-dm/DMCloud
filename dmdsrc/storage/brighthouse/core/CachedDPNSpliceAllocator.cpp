...
{
	mytimer dpn_loadtm,dpn_readtm,dpn_buildtm;
	dpn_loadtm.Start();
	...
	for ( int i( no_splices ); i < new_; ++ i ) {
		if ( ! vec_[i - offset] ) {
			...
			if ( !in_cache( i - offset, trans ) )
				...
				BHASSERT_WITH_NO_PERFORMANCE_IMPACT( !"Empty splice in cache" );
				vec_[ i - offset ] = get_from_cache( i - offset, trans );
			}
		}
	}
	...
	if(rccontrol.isOn() && RIAK_OPER_LOG>=1) {
		rclog<<lock<<"load dpn  "<<_attr->table_number<<"/"<<_attr->attr_number<< 
		": t"<< F2MS(dpn_loadtm.GetTime())<<"ms."<<
		": r"<< F2MS(dpn_readtm.GetTime())<<"ms."<<
		": b"<< F2MS(dpn_buildtm.GetTime())<<"ms."<<unlock;
	}
}
