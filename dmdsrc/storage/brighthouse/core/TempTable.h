...
public:
	...
	public:
		...
		void CreateBuffer(_uint64 size, ConnectionInfo *conn = NULL);
...
private:
	TempTable(const TempTable &, bool for_subq = false, bool for_rough_query = false);
	TempTable(JustATable * const , int alias, Query* q);

	TempTablePtr CreateMaterializedCopy(bool translate_order);		// move all buffers to a newly created TempTable, make this VC-pointing to it; the new table must be deleted by DeleteMaterializedCopy()
...
public:
	...
	void					RoughAggregateSum(VirtualColumn *vc, _int64 &min_val, _int64 &max_val, vector<Attr*> &group_by_attrs, bool nulls_only, bool distinct_present);
	...
	void					LockPackForUse(unsigned attr, _int64 *pahead, unsigned pack_no, ConnectionInfo& conn,Descriptor *pdesc,uint range,void *pf);
	void					UnlockPackFromUse(unsigned attr, unsigned pack_no) {}
	void 					PreloadPack(unsigned attr, _int64 *pahead,ConnectionInfo& conn,Descriptor *pdesc);
	void 					WaitPreload(unsigned attr,ulong transid);
	...
	void					ReserveVirtColumns(int no) {assert(!no || no> virt_cols.size()); virt_cols.resize(no); virt_cols_for_having.resize(no); }
...
protected:
	...
	bool					for_subq;	// true if this is a temptable copy for subq execution
...
public:
	...
	static TempTablePtr		Create(const TempTable &, bool for_subq = false, bool for_rough_query = false);
	static TempTablePtr		Create(JustATable * const , int alias, Query* q);
