...
public:
	...
	virtual double GetValueD(unsigned char *buf)							{ return *((double*)buf); }
	virtual _int64 GetValue64(unsigned char *buf)							{ return *((_int64*)buf); }	// double passed as 64-bit code
	virtual void SetAggregatePackMin(_int64 par1)				 			{ pack_min = *((double*)&par1); }
	virtual bool AggregatePack(unsigned char *buf);
	...
	virtual bool UpdateStatistics(unsigned char *buf)
	{
		if(*((double*)buf) == NULL_VALUE_D)
			null_group_found = true;
		else if(stat_max == NULL_VALUE_D || *((double*)buf) > stat_max)
			stat_max = *((double*)buf);
		return false;
	}
	virtual bool PackCannotChangeAggregation()
	{
		assert(stats_updated);
		return !null_group_found && pack_min != NULL_VALUE_D && stat_max != NULL_VALUE_D && pack_min >= stat_max;
	}
private:
	double stat_max;
	double pack_min;
...
public:
	...
	virtual double GetValueD(unsigned char *buf)							{ return *((double*)buf); }
	virtual _int64 GetValue64(unsigned char *buf)							{ return *((_int64*)buf); }	// double passed as 64-bit code
	virtual void SetAggregatePackMax(_int64 par1)				 			{ pack_max = *((double*)&par1); }
	virtual bool AggregatePack(unsigned char *buf);
	...
	virtual bool UpdateStatistics(unsigned char *buf)
	{
		if(*((double*)buf) == NULL_VALUE_D)
			null_group_found = true;
		else if(stat_min == NULL_VALUE_D || *((double*)buf) < stat_min)
			stat_min = *((double*)buf);
		return false;
	}
	virtual bool PackCannotChangeAggregation()
	{
		assert(stats_updated);
		return !null_group_found && pack_max != NULL_VALUE_D && stat_min != NULL_VALUE_D && pack_max <= stat_min;
	}
private:
	double stat_min;
	double pack_max;
