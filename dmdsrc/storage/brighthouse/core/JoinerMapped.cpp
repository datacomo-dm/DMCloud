...
void JoinerMapped::ExecuteJoinConditions(Condition& cond)
{
	...
	while(mit.IsValid()) {
		...
		if(mit.PackrowStarted()) {
			...
			_int64 local_min = vc2->GetMinInt64(mit);
			_int64 local_max = vc2->GetMaxInt64(mit);
			if(local_min == NULL_VALUE_64 || local_max == NULL_VALUE_64 ||		// NULL_VALUE_64 only for nulls only
				map_function->ImpossibleValues(local_min, local_max)) {
					omit_this_packrow = true;
			}
			packrows_matched++;
			if(single_filter_dim != -1 && local_min != NULL_VALUE_64 && local_max != NULL_VALUE_64
				&& map_function->CertainValues(local_min, local_max) && vc2->GetNoNulls(mit) == 0) {
...
bool OffsetMapFunction::Init(VirtualColumn *vc, MIIterator &mit)
{
	...
	if(key_table_min == NULL_VALUE_64 || key_table_max == NULL_VALUE_64 || (key_table_min < -2000000000 && key_table_max > 2000000000))
		return false;
	_int64 span = key_table_max - key_table_min + 1;
	if(span > 32 * MBYTE || (span < mit.NoTuples() && !vc->NullsPossible()))
		return false;
