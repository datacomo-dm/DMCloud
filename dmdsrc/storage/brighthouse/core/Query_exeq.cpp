		...
		try	{
			...
			switch(step.type) {
				...
				case CompiledQuery::TMP_TABLE:
					assert(step.t1.n < 0);
					ta[-step.t1.n - 1] = TempTable::Create(ta[-step.tables1[0].n - 1].get(), step.tables1[0].n, this);
				...
				case CompiledQuery::UNION:
					...
					assert(ta[-step.t2.n - 1]->TableType() == TEMP_TABLE && ta[-step.t3.n - 1]->TableType() == TEMP_TABLE);
					if(step.t1.n != step.t2.n)
						ta[-step.t1.n - 1] = TempTable::Create(*(TempTable *)ta[-step.t2.n - 1].get());
					if(IsRoughQuery()) {
