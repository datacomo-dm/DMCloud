...
int Query::AddJoins(List<TABLE_LIST>& join, TabID& tmp_table, std::vector<TabID>& left_tables, std::vector<TabID>& right_tables, bool in_subquery, bool& first_table /*= true*/)
{
#ifdef PURE_LIBRARY
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
	return 0;
...
int Query::AddOrderByFields(ORDER* order_by, TabID const& tmp_table, int const group_by_clause)
{
	...
	for(; order_by; order_by = order_by->next) {
		...
		if(!IsFieldItem(item) && !IsAggregationItem(item) && item->type() != Item::SUBSELECT_ITEM) {
			MysqlExpression* expr = NULL;
...
int Query::AddColumnForPhysColumn(Item* item, const TabID& tmp_table, const ColOperation oper, const bool distinct, bool group_by, const char* alias)
{
	...
	if(item->type() == Item_bhfield::get_bhitem_type() && IsAggregationItem(dynamic_cast<Item_bhfield *>(item)->OriginalItem()))
		return ((Item_bhfield*)item)->varID.col;
		...
		bool ignore_minmax)
{
	...
	for(SELECT_LEX* sl = selects_list; sl; sl = sl->next_select()) {
		if(sl!=selects_list) { //only in case of unions this is set
			SetLimit(sl->join->unit->global_parameters, 0, global_offset_value, (_int64&)global_limit_value);
			global_order = &(sl->join->unit->global_parameters->order_list);
		};
