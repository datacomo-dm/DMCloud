...
void RCAttr::EvaluatePack_IsNull(MIUpdatingIterator &mit, int dim)
{
	...
	ushort objs=dpns[pack].no_objs;
	if(dpns[pack].pack_mode != PACK_MODE_TRIVIAL && dpns[pack].no_nulls != 0) {		// nontrivial pack exists
		AttrPack *attrpack=dpns[pack].GetPack();
		do {
			if(mit[dim] != NULL_VALUE_64 && !attrpack->IsNull(mit.GetCurInpack(dim)))
				mit.ResetCurrent();
			++mit;
			while(mit.IsValid() && mit.GetCurInpack(dim)>objs && !mit.PackrowStarted()) {
				mit.ResetCurrent();
				++mit;
			}
...
void RCAttr::EvaluatePack_Like(MIUpdatingIterator &mit, int dim, Descriptor& d)
{
	...
	AttrPackS *p = (AttrPackS*)dpns[pack].GetPackS();
	...
	if(p->IsCQ()) {
		CRResultPtr &list=p->GetCRResult();
		if(list->size()>0)
			mit.ResetCurrentPackBut(list);
		else
			mit.ResetCurrentPack();
		mit.NextPackrow();
		return;
...
void RCAttr::EvaluatePack_InNum(MIUpdatingIterator& mit, int dim, Descriptor& d)
{
	...
	DPN& dpn = dpns[pack];
	AttrPackN* p = (AttrPackN*)dpn.GetPackN();
	if(p->IsCQ()) {
		CRResultPtr &list=p->GetCRResult();
		if(list->size()>0)
			mit.ResetCurrentPackBut(list);
...
void RCAttr::EvaluatePack_BetweenString(MIUpdatingIterator &mit, int dim, Descriptor& d)
{
	...
	//riak CR query pack stored in list of part-range packs
	AttrPackS* p = (AttrPackS*)dpns[pack].GetPackS();
	ushort objs=dpns[pack].no_objs;
	if(p == NULL) {			// => nulls only
		mit.ResetCurrentPack();
	...
	int inpack;
	bool res;
	// Riak has done the work on condition query
	if(p->IsCQ()) {
		CRResultPtr &list=p->GetCRResult();
		if(list->size()>0)
			mit.ResetCurrentPackBut(list);
...
void RCAttr::EvaluatePack_BetweenReal(MIUpdatingIterator &mit, int dim, Descriptor& d)
{
	...
	AttrPackN *p = (AttrPackN*)dpns[pack].GetPackN();
	if(p->IsCQ()) {
		CRResultPtr &list=p->GetCRResult();
		if(list->size()>0)
			mit.ResetCurrentPackBut(list);
...
void RCAttr::EvaluatePack_AttrAttr(MIUpdatingIterator &mit, int dim, Descriptor& d)
{
	...
	AttrPackN *p1 = (AttrPackN*)dpns[pack].GetPackN();
	AttrPackN *p2 = (AttrPackN*)(a2->dpns[pack].GetPackN());
	ushort objs=dpns[pack].no_objs;
		...
		d1.GetCollation().collation == d2.GetCollation().collation) {
			...
			if(d1.op == O_BETWEEN && d2.op == O_BETWEEN) {
				if(RequiresUTFConversions(my_coll)) {
					...
					if(d1max.IsNull() || CollationStrCmp(my_coll, d2max, d1max, O_LESS)) {
						swap(d1.val2, d2.val2);
						swap(d1max, d2max);
					}
