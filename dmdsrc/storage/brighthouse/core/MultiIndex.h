...
public:
	...
	void AddDimension_cross(_uint64 size,std::map<int,int> *pnfblocks=NULL,bool adjtumples=false);		// calculate a cross product of the previous value of index and the full table (trivial filter) of 'size' objects
	...
	{ assert(dim<no_dimensions && dim>=0); return group_for_dim[dim]->GetFilter(dim); }
	...
	{ assert(dim<no_dimensions && dim>=0); return group_for_dim[dim]->GetUpdatableFilter(dim); }
	bool NullsExist(int dim)				{ return group_for_dim[dim]->NullsPossible(dim); }	// return true if there exist any 0 value (always false for virtual dimensions)
	...
	bool IsOrderable(int dim)				{ return group_for_dim[dim]->IsOrderable(); }
	...
	bool IsForgotten(int dim)			{ return !group_for_dim[dim]->DimEnabled(dim); }	// true if the dimension is forgottrn (not valid for getting value)
	...
	string Display();		// MultiIndex structure: f - Filter, i - IndexTable
...
private:
	...
	vector<int> ListInvolvedDimGroups(DimensionVector &v);	// List all internal numbers of groups touched by the set of dimensions
