...
public:
	...
	public:
		...
		virtual uint GetRangeInPack(int dim,int pack,void **pf) { return RANGEFULL;}
		...
		virtual void ResetCurrentPackrowBut(CRResultPtr &list)		{}				// reset the whole current packrow
...
{
public:
	DimensionGroupFilter(int dim, _int64 size,std::map<int,int> *pnfblocks=NULL);
	...
	public:
		...
		virtual uint GetRangeInPack(int dim,int pack,void **pf) { return fi.GetRangeInPack(pack,pf);}
		...
		virtual void ResetCurrentPackrowBut(CRResultPtr &list)		{fi.ResetCurrentPackrowBut(list);}				// reset the whole current packrow
