...
FilterOnesIterator::FilterOnesIterator() : buffer(max_ahead)
{
	...
	packs_to_go = MAX_ROW_NUMBER >> 16; //all packs
	packs_done = 0;
	delayedblock=-1;
	delayedfrom=-1;
	delayedto=-1;
...
void FilterOnesIterator::Commit() {
	if(delayedblock>-1) {
		f->ResetBetween(delayedblock,delayedfrom,delayedblock,delayedto);
	}
}
...
void FilterOnesIterator::ResetDelayed()				// reset (delayed) the current position of filter
	{
		/*
		BUG on TRUE delayed reset:
		    PackGuadian unlock source pack give a wrong pack_no for sub routine ,but it's value is ok in PackGuadian
		    
		if(delayedblock!=iterator_b) {
			if(delayedblock>-1)
				Commit();
			curobjs=f->GetBlockObjs(iterator_b)-1;
			delayedblock=iterator_b;
			delayedfrom=0;
		}
		else if(delayedto+1!=iterator_n)
		{
			Commit();
			delayedfrom=iterator_n;
		}
		delayedto=iterator_n;
		if(iterator_n==curobjs) 
			Commit();
...
_int64 FilterOnesIterator::GetPackSizeLeft()	// how many 1-s in the current block left (including the current one)
{
	if(!valid)
		return 0;
	if(cur_block_full) {
//		if(iterator_b == f->no_blocks - 1) {
//			return f->no_of_bits_in_last_block - iterator_n;
//		}
//		return 65536 - iterator_n;
		 return f->BlockObj(iterator_b)-iterator_n;
	}
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT(f->block_status[iterator_b] != f->FB_EMPTY);
...
uint FilterOnesIterator::GetRangeInPack(int p,void **pf)
{
	if(f->block_status[p] == f->FB_EMPTY || f->block_status[p] == f->FB_FULL ||
		(f->block_status[p] == f->FB_MIXED && (f->blocks==NULL || f->blocks[p]==NULL) ) )
		return 	RANGEFULL;
	return f->blocks[p]->GetRange(pf);
}

bool FilterOnesIterator::IteratorBpp()
{
	//rewindtopack will --packs_donw, so SetPackstoGo(reset to 0) became -1. not actually packs_done for intial by rewind
	if(packs_to_go==1) return false;
	iterator_n = 0;
	if(buffer.Empty()) {
		iterator_b++;
...
//	if(iterator_n == 65535 || (iterator_b == f->no_blocks - 1 && iterator_n == f->no_of_bits_in_last_block - 1)) {
	if(iterator_n >= f->BlockObj(iterator_b)-1) {
		inside_pack = false;
		iterator_n = 0;
		prev_block = -1;
	} else
...
int FilterOnesIteratorOrdered::Lookahead(int n)
{
	if(n == 0)
		return iterator_b;
	// use NEW Next method instead of change po current position which will referenced in other(s)
	return po->Next(n);
	/*
	if(buffer.Elems() >= n)
		return buffer.Nth(n-1);
