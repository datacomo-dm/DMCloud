...
int RCEngine::Convert(int& is_null, Field* field, void* out_ptr, RCDataType& rcitem)
{
	...
	switch(field->type()) {
		...
		case MYSQL_TYPE_BLOB: {
			BHASSERT_WITH_NO_PERFORMANCE_IMPACT(dynamic_cast<RCBString*>(&rcitem));
			((Field_blob*)field)->set_ptr(((RCBString&)rcitem).len, (uchar*)((RCBString&)rcitem).val);
...
int RCEngine::Convert(int& is_null, my_decimal* value, RCDataType& rcitem)
{
#ifdef PURE_LIBRARY
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
	return 0;
...
int RCEngine::Convert(int& is_null, _int64& value, RCDataType& rcitem)
{
	if(rcitem.IsNull())
		is_null = 1;
	else {
