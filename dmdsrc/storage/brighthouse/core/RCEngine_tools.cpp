...
bool RCEngine::IsBHRoute(THD * thd, TABLE_LIST* table_list, SELECT_LEX* selects_list, int &in_case_of_failure_can_go_to_mysql, int with_insert)
{
	...
	if (file) {//it writes to a file
		...
		if(!get_parameter(thd, BH_DATAFORMAT, param, s_res)) {
			...
			//	"BH_DATAFORMAT parameter is not set, 'MYSQL' value assumed, query executed by MySQL engine.");				
			//return false;
			//my_message(ER_SYNTAX_ERROR, "Brighthouse specific error: Unknown value of BH_DATAFORMAT parameter", MYF(0));
			//"BH_DATAFORMAT parameter is not set, 'TXT_VARIABLE' value assumed.");			
			return true;
		}
	}
...
auto_ptr<IOParameters> RCEngine::CreateIOParameters(const string& path)
{
	...
	auto_ptr<IOParameters> iop(new IOParameters(data_dir, data_path));
...
BHError RCEngine::GetIOParameters(IOParameters& io_params, THD& thd, sql_exchange& ex, TABLE* table, bool for_exporter)
{
	...
	String *line_start = ex.line_start, *line_term = ex.line_term;	
...
//			vector<string> names;
//			DataFormat::GetFormatsNames(names);
//			string message("Data format for import not set. Please set @bh_dataformat to one of the following supported formats: ");
//			for(int i = 0, size = names.size(); i < size; i++)
//				message += "'" + to_lower_copy(names[i]) + "', ";
//			message += "'" + names[names.size() - 1] + "mysql";
//			return BHError(BHERROR_WRONG_PARAMETER, message);
			//BHERROR("BH_DATAFORMAT parameter not set!");
	...
	if(IsPipe(ex.file_name)) {
		dm_strlcpy(name, ex.file_name, sizeof(name));
	...
	if(Configuration::GetProperty(Configuration::UseMySQLImportExportDefaults)) {
		...
		io_params.SetParameter(DELIMITER, *field_term->c_ptr());
		...
		if(!(*field_term->c_ptr() == '\t' && field_term->alloced_length() == 0))
			io_params.SetParameter(DELIMITER, *field_term->c_ptr());
	...
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT(line_term != 0);
	io_params.SetParameter(LINE_TERMINATOR, string(line_term->ptr()));
	if(((thd.lex)->sql_command == SQLCOM_LOAD) && line_term->alloced_length() != 0)
		unsupported_syntax = true;
	...
	//	return BHERROR_UNSUPPORTED_SYNTAX;	
