...
int RCEngine::Execute(THD* thd, LEX* lex, select_result* result_output, SELECT_LEX_UNIT* unit_for_union)
{
	...
	Query* query = new Query(cur_connection);
	CompiledQuery cqu;
	...
	try	{
		...
		if(unit_for_union != NULL)	{
			...
			if(export_file_name)
				sender = new ResultExportSender(unit_for_union->thd, result_output, unit_for_union->item_list);
			else
				sender = new ResultSender(unit_for_union->thd, result_output, unit_for_union->item_list);
		} else {
			if(export_file_name)
				sender = new ResultExportSender(selects_list->join->thd, result_output, selects_list->item_list);
			else
				sender = new ResultSender(selects_list->join->thd, result_output, selects_list->item_list);
		}

/*
		for(std::map<int, std::vector<int> >::const_iterator it = cqu.GetTableIds().begin(); it != cqu.GetTableIds().end(); ++it) {
			//for each RCTable defined by path
			cerr<< (*it).first << endl;

			std::set<int> cols = cqu.GetColumnsPerTable((*it).first);

//			std::set<int> cols;
//			for(int a = 0; a < (*it).second.size(); ++a) {
//				std::vector<int>& c = cqu.GetColumnsPerTable((*it).second[a]);
//				cols.insert(c.begin(), c.end());
//
//			}
			for(std::set<int>::const_iterator it =cols.begin(); it!=cols.end(); ++it)
				//columns from the RCTable used by query
...
*/
		...
		result = query->Preexecute(cqu, sender);
		BHASSERT( result != NULL, "Query execution returned no result object" );
		if(query->IsRoughQuery())
			result->RoughMaterialize(false, sender);
		else
			result->Materialize(false, sender);

		query->UnlockPackInfoFromUse();
