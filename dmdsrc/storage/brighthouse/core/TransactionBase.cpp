...
{
    if(GetTableShared(table_path, table_share)){
...
AttrPackPtr
TransactionBase::GetAttrPack(Descriptor *pdesc,const PackCoordinate& pack_coord, PackAllocator &a,void *pf)
{
	AttrPackPtr ret;
	{
	...
	if (tv.table && (uint)(pc_dp( pack_coord ) + 1) >= tv.initial_no_packs) {
		ret = m_localCache.GetObject<AttrPack>(pack_coord, bind( &AttrPackLoad, pdesc,boost::ref(pack_coord), boost::ref( a ),pf ));
	} else{
		ret = m_globalCache->GetObject<AttrPack>(pack_coord, bind( &AttrPackLoad, pdesc,boost::ref(pack_coord), boost::ref( a ),pf ) );
	}
...
void TransactionBase::VerifyTablesIntegrity()
{
	...
	for (iter = m_modified_tables.begin(); iter != m_modified_tables.end(); ++iter) {
		...
		if (tab->Verify() != 1) {
			std::string err_msg = "RCEngine::Commit  Data integrity is broken in table ";
			err_msg += iter->first;
