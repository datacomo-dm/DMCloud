...
// new_mind and outer_tuples (the counter) are used only for outer joins
{
	...
	for(int i = 0; i < cond_hashed; i++)
		vc1[i]->InitPrefetching(mit,cond_hashed);
	if(watch_traversed)
...
_int64 JoinerHash::MatchDim(MINewContents &new_mind, MIIterator &mit)
{
	...
	for(int i = 0; i < cond_hashed; i++)
		vc2[i]->InitPrefetching(mit,cond_hashed);
	// 允许多线程并发预读取DataPack
	mit.SetPackMttLoad();
	while(mit.IsValid()) {
		if(m_conn.killed())
...
void JoinerHash::InitOuter(Condition& cond)
{
	...
	if(!outer_dims.IsEmpty()) {
		if(traversed_dims.Includes(outer_dims)) {
			...
			outer_filter = new Filter(mind->NoTuples(matched_dims), nfblocks,true);	// the filter updated for each matching
		}
		else if(matched_dims.Includes(outer_dims)) {
			watch_traversed = true;
			outer_filter = new Filter(jhash.NoRows(),nfblocks);			// the filter reused for each traverse
		}
