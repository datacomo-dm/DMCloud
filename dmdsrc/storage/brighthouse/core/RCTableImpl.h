...
public:
	...
	virtual ~RCTableImpl();
	int MergePackHash(uint s_id);
	int MergePackIndex(uint s_id);
	...
	void SaveHeader(bool truncatepart=false,_int64 new_no_obj=0); // save table definition
	...
	void FillDecompositions(vector<std::string>& decompositions);
...
protected:
	...
	virtual void LoadAttribute(int attr_no,bool loadapindex=true);
...
public:
    void ClearLoadCtlFile(const long conn_id=0);  // 删除connect对应的loadinfo.ctl文件
	...
	virtual Iterator Begin(const std::vector<bool>& attrs)			{ 
		std::map<int,int> nfblocksempty;
		return Iterator::CreateBegin(*this,	FilterPtr(new Filter(NoObj(), nfblocks==NULL?nfblocksempty:*nfblocks,true)), attrs); 
	}
