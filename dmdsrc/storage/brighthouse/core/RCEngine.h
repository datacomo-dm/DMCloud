...
enum BHEngineReturnValues {LD_Successed = 100, LD_Failed = 101, LD_Continue = 102,LD_Locked=103};
...
private:
	void CreateThreadParams(THD& thd);
    ...
    void get_type_file(const char* dirpath,const char* filetype, std::vector<std::string>& file_list,bool recursion = true); 
    int  is_type_file(const char* filepath,const char* filetype);

    //>> begin: fix dma-1105 add by liujs 
    void clear_loading_file();
    DMThreadGroup  thd_clear_loading;
    //<< end: fix dma-1105

    //>> begin: fix dma-1180
    void clear_dpadmin_lst_file();    
    //end: fix dma-1180
...
public:
    static int Convert(int & is_null, Field *field, void *out_ptr, RCDataType& rcitem);
    static int Convert(int & is_null, my_decimal *value, RCDataType& rcitem);
    static int Convert(int & is_null, int64 & value, RCDataType& rcitem);
...
public:
    ...
    static std::pair<std::string, std::string> GetDatabaseAndTableNamesFromFullPath(const string& full_path);
	...
	static DWORD WINAPI insert_th_func(void* f) {
...
private:
    void AdjustTimezone(RCDateTime & dt);
};
