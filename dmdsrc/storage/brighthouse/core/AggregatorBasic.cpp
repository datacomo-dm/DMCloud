...
void AggregatorAvg64::PutAggregatedValue(unsigned char *buf, _int64 v, _int64 factor)
{
	...
	*((double*)buf) += double(v) * factor;
	if(!warning_issued && (*((double*)buf) >  9.223372037e+18 || *((double*)buf)< -9.223372037e+18)) {
		push_warning(&ConnectionInfoOnTLS.Get().Thd(), MYSQL_ERROR::WARN_LEVEL_NOTE, ER_UNKNOWN_ERROR, "Values rounded in average() in Brighthouse");
		warning_issued = true;
	}
	*((_int64*)(buf + 8)) += factor;
#else
...
{
	if(*((double*)src_buf) == NULL_VALUE_D)
		return;
	if( *((double*)buf) == NULL_VALUE_D ||
		*((double*)buf) > *((double*)src_buf)) {
			stats_updated = false;
