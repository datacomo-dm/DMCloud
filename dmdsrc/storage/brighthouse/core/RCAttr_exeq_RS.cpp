...
// file RCAttr_exeq_RS.cpp
RSValue RCAttr::RoughPackIndexCheck(MIUpdatingIterator &mit, int dim, Descriptor& d, bool additional_nulls_possible,std::vector<int> &packs)
{
#ifdef PURE_LIBRARY
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
	return RS_SOME;
#else
	if(ct.IsPackIndex() && current_state == SESSION_READ && ldb_index==NULL) {
			ldb_index=new AttrIndex(attr_number,path.c_str());
			ldb_index->LoadHeader();
			ldb_index->LoadForRead();
	}
	if(ldb_index==NULL || !ldb_index->Valid()) return RS_UNKNOWN;
	if(d.op == O_FALSE)
		return RS_NONE;
	else if(d.op == O_TRUE)
		return RS_ALL;
	else if(d.op == O_IS_NULL || d.op == O_NOT_NULL || d.op == O_LIKE || d.op == O_NOT_LIKE ) 
		return RS_UNKNOWN;
	//can leveldb used to assists to filter reverse? no
	bool reverse=(d.op==O_NOT_BETWEEN  || d.op == O_NOT_IN);
	if(reverse) return RS_UNKNOWN;
	//std::vector<int> selected_packs;
	...
	if(d.IsType_AttrValOrAttrValVal()) {
		...
		if((d.op == O_IN || d.op == O_NOT_IN) && PackType() == PackS) {
			...
			if((mvc->IsConst()) && (mvc->NoValues(mit) > 0 && mvc->NoValues(mit) < 64) && !RequiresUTFConversions(d.GetCollation())) {
					...
					for(MultiValColumn::Iterator it = mvc->begin(mit), end = mvc->end(mit); (it != end) && (res == RS_NONE); ++it) {
						...
						if(v1.size()<1) return RS_UNKNOWN;;
						leveldb::Slice keyslice(v1.Value(),v1.size());
						ldb_index->Get(keyslice, packs);
					}
			}
			else return RS_UNKNOWN;
			...
			//TODO : to implement one of values is null?
			if(vmin.IsNull() || vmax.IsNull())			// comparing with null - always false
				return RS_UNKNOWN;
			leveldb::Slice s1(vmin.Value(),vmin.size()),s2(vmax.Value(),vmax.size());
			ldb_index->GetRange(s1,s2,packs);
			...
			ldb_index->GetRange(v1,v2,packs);

		}
	} else {
		// TODO : any method to process packs op packs ?
		//not 	this(attr) op (value or values)
		//but  this(attr) op another attr--> not a single value or a list of value, but a pack to operator with another pack
		if(!d.encoded)
			return RS_UNKNOWN;
		// There's not a method to process on this situation for level db yet
		return RS_UNKNOWN;
	}
	if(packs.size()<1) return RS_NONE;
	/* remove merge to caller, as a descriptor maybe combine of OR/AND,multiple packs query need to merge at higher level
	// Merge index filter result
	std::sort(selected_packs.begin(),selected_packs.end());
	// mit must has included all selected_packs elements!
	// selected_packs is in order 
	uint i=0;
	int maxsel=selected_packs.size()-1;
	while(mit.IsValid()) {
		// continue on previous 
		uint curpack=mit.GetCurPackrow(dim);
		// maybe duplicate pack value in selected_packs ,but not duplicate in mit
		while(curpack==selected_packs[i]) {
				//can we assists to filter reverse? no
				//if(reverse)
				//	mit.ResetCurrentPack();
				if(i==maxsel) break;
			  i++;
		}
		//if(!reverse && curpack!=selected_packs[i]) {
		if(curpack!=selected_packs[i]) {
			mit.ResetCurrentPack();
		}
		mit.NextPackrow();
	}
	...
	mit.Commit();
	if(rccontrol.isOn()) {
		rccontrol.lock() <<"PackIndex on Col ("<<Name()<< ") got "<<selected_packs.size()<< " Packs ." << unlock;
	}
	*/
	return RS_SOME;
#endif
...
RSValue RCAttr::RoughCheck(int pack, Descriptor& d, bool additional_nulls_possible)
{
#ifdef PURE_LIBRARY
	BHERROR("NOT IMPLEMENTED! Depends on MySQL code.");
	return RS_SOME;
		...
		if(d.op == O_IS_NULL || d.op == O_NOT_NULL) {
			...
			if(res == RS_SOME && min(pattern_prefix, pack_prefix) < pat.len && !RequiresUTFConversions(d.GetCollation())) {
				...
				if( !(pattern_for_cmap.len == 1 && pattern_for_cmap[0] == '%') ) {		// i.e. "%" => all is matching
                    ...
                    //>> begin: fix dma-1229, modify by liujs
                    RCBString pre_fix_info = GetPrefix(pack);   
                    if(pattern_fixed_prefix>0 && pattern_fixed_prefix<pre_fix_info.len){ 
                        for(int i=0;i<pattern_fixed_prefix;i++){// 例如: like '0__%' 需要逐个匹配
                            if(pat[i] != pre_fix_info[i]){
                                res = RS_NONE;
                                break;
                            }
                        }       
                    ...
                    // res = RS_ALL; // delete by liujs for dma-1229
			    }
			...
			if(res == RS_SOME && vmin.len >= pack_prefix && vmax.len >= pack_prefix && !RequiresUTFConversions(d.GetCollation())) {
                ...
                //>> begin: try to fix dma-1199,add by liujs
                RCBString min_s((char*)&dpn.local_min,8); // dpn min string
                RCBString max_s((char*)&dpn.local_max,8); // dpn max string
                
                if(vmin < min_s){ 
                    unsigned char t_min[64]; // cmap 字符串长度最大支持64字符
                    memset(t_min,0x0,64);
                    RCBString t_smin((char*)t_min,vmax.len); // 只要vmax的长度即可
                    vmin = t_smin;
                }

                if(vmax > max_s){
                    unsigned char t_max[64]; // cmap 字符串长度最大支持64字符
                    memset(t_max,0xff,64);
                    RCBString t_smax((char*)t_max,vmax.len); // 只要vmax的长度即可
                    vmax = t_smax;
                }        
	...
	if(ATI::IsRealType(TypeName())) {
		...
		for(int p=0; p < NoPack(); p++) {			// minimum of nonempty packs
			...
			if((f == NULL || !f->IsEmpty(p)) && (rf == NULL || rf[p] != RS_NONE)) {
				if(*(double*)(&res) > *(double*)(&dpn.local_min))
					res = dpn.local_min;
			}
		}
	} else {
...
_int64 RCAttr::RoughMax(Filter *f, RSValue* rf)		// f == NULL is treated as full filter
{
	...
	if(ATI::IsRealType(TypeName())) {
		...
		for(int p=0; p < NoPack(); p++) {		// minimum of nonempty packs
			DPN const& dpn( dpns[p] );
			if((f == NULL || !f->IsEmpty(p)) && (rf == NULL || rf[p] != RS_NONE)) {
				if(*(double*)(&res) < *(double*)(&dpn.local_max))
					res = dpn.local_max;
			}
		}
	} else {
...
_uint64 RCAttr::ApproxDistinctVals(bool incl_nulls, Filter *f, RSValue* rf, bool outer_nulls_possible)
{
	...
	if(TypeName()==RC_DATE) {
		...
		if(span < 100000) {				// use Histograms to calculate exact number of distinct values
			...
			Filter values_present(span,nfblocks);
	...
	for(int pack = 0; pack < npack; pack++)
	{
		...
		if(dpn.no_objs+1 != dpn.no_nulls) {
			if(PackType()==PackN && !ATI::IsRealType(TypeName())) {
				...
				bool nonnegative = false;						// default: may be negative
				...
				sprintf(line_buf + strlen(line_buf), " %16g %16g %15g  -", 
					*((double*)(&dpn.local_min)), *((double*)(&dpn.local_max)), GetSumD(pack));
			} else {		// PackType = PackS
				char smin[10];
				char smax[10];
				...
				smin[8]='\0';
				smax[8]='\0';
				snprintf(line_buf + strlen(line_buf), sizeof(line_buf) - strlen(line_buf), "  %8s  %8s %5d ", smin, smax, GetActualSize(pack));
