...
public:
	...
	ColumnType() :	type(RC_INT), precision(0), scale(0), is_lookup(false),is_packindex(false),is_bloomfilter(false),
					compress_type(Compress_DEFAULT),nulls_mode(AS_MISSED),
					internal_size(4), display_size(11), collation(DTCollation())
	  {}

  //! create ColumnType object and calculate proper internal_ and display_size
	ColumnType(AttributeType t, NullMode nulls = AS_MISSED, bool lookup = false, int prec = 0, int sc = 0, bool packindex=false,bool bloomfilter=false,int _compresstype=Compress_DEFAULT,DTCollation collation = DTCollation()):
		type(t), precision(prec), scale(sc), is_lookup(lookup),is_packindex(packindex),is_bloomfilter(bloomfilter),
		compress_type(_compresstype),nulls_mode(nulls),internal_size(InternalSize()), display_size(ATI::TextSize(t, prec, sc, collation)),
		collation(collation)
		{}

	void Initialize(AttributeType t, NullMode nulls, bool lookup, int prec, int sc,bool packindex,bool bloomfilter,
		int _compress_type,DTCollation collation = DTCollation()) {
		type = t;
		nulls_mode = nulls;
		is_lookup = lookup;
		is_packindex=packindex;
		is_bloomfilter=bloomfilter;
		compress_type = _compress_type;
		precision = prec;
		scale = sc;
	...
	inline bool IsPackIndex() const { return is_packindex; }
	inline bool IsBloomFilter() const { return is_bloomfilter; }
	inline	 int GetCompressType()const {return compress_type;}
