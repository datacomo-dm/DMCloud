...
public:
	...
	void TmpTable(TabID& t_out, const TabID& t1); 
	...
	void Add_Order(const TabID& p_t, ptrdiff_t p_c) {
		int a_c(static_cast<int>(p_c)); if(p_c == a_c) return this->Add_Order(p_t, AttrID(-abs(a_c)), a_c < 0);
	}
	...
	void Print();			// display the CompiledQuery
