...
void GroupByWrapper::Initialize(_int64 upper_approx_of_groups)
{
	//MEASURE_FET("GroupByWrapper::Initialize(...)");
	gt->Initialize(upper_approx_of_groups);
...
{
	_int64 old_groups = gt->GetNoOfOccupiedRows();
	gt->Merge(*(sec.gt));
	packrows_omitted += sec.packrows_omitted;
	packrows_part_omitted += sec.packrows_part_omitted;
	no_groups += gt->GetNoOfOccupiedRows() - old_groups;		// note that no_groups may be different than gt->..., because it is global
}
...
bool GroupByWrapper::AggregatePackInOneGroup( int attr_no, MIIterator &mit, _int64 uniform_pos, _int64 rows_in_pack, _int64 factor)
{
	...
	if(virt_col[attr_no] && virt_col[attr_no]->RoughNullsOnly(mit) && gt->AttrAggregator(attr_no)->IgnoreNulls())
...
bool GroupByWrapper::AddPackIfUniform(int attr_no, MIIterator &mit)
{
	if(virt_col[attr_no] && virt_col[attr_no]->GetPackOntologicalStatus(mit) == UNIFORM && !mit.NullsPossibleInPack()) {
		...
		pack_not_omitted[attr_no] = true;	// just to make locking possible
		LockPack(attr_no, mit);
		gt->PutGroupingValue(attr_no, mit);
		return true;
	}
	return false;
}
...
bool GroupByWrapper::PackWillNotUpdateAggregation(int i, MIIterator &mit)	// false, if counters can be changed
{
	...
	if(gt->GetNoOfOccupiedRows() > 1024)
		return false;
	...
	if(gt->AttrAggregator(i)->PackAggregationNeedsMin() && virt_col[i]) {
	...
	if(gt->AttrAggregator(i)->PackAggregationNeedsMax() && virt_col[i]) {
...
void GroupByWrapper::DistinctlyOmitted(int attr, _int64 obj)
{
	...
	gt->AddCurrentValueToCache(attr, distinct_watch.gd_cache[attr]);		// note: ignored if no place left
...
void DistinctWrapper::InitTuples(_int64 n_obj)
{
	no_obj = n_obj;
	for(int i = 0; i < no_attr; i++ )
		if(is_dist[i]) {
			// for now - init cache with a number of objects decreased:
			...
			if(no_obj > limit_for_small_distincters)	// more objects - prepare for caching omitted objects
				gd_cache[i].SetNoObj(no_obj/10 + (no_obj - limit_for_small_distincters));
			else										// less objects - rarely omitted
				gd_cache[i].SetNoObj(no_obj/5 + 1);
			assert(f[i] == NULL);
			std::map<int,int> nfblocks;
			f[i] = new Filter(no_obj,nfblocks);
		}
}
