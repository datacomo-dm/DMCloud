...
bool ParameterizedFilter::RoughUpdateMultiIndex()
{
	...
	if(!false_desc) {
		...
		// after rougth filter by DPN&RSI, continue to filter by leveldb& bloomfilter
		for(uint i = 0; i < descriptors.Size(); i++) {
			if(!descriptors[i].done && !descriptors[i].IsDelayed() && descriptors[i].IsInner() &&
				descriptors[i].GetJoinType() == DT_NON_JOIN) {

					if(descriptors[i].op== O_TRUE) continue;
					...
					std::vector<int> selected_packs_orig;
					std::vector<int> selected_packs;
					std::set<int> selected_packs_set;
					// got RS_UNKNOWN: wether no packindex or packindex can not apply to this operation
					RSValue pi_rsvalue=descriptors[i].EvaluateRoughlyPackIndex(mit,selected_packs_orig);

					if(pi_rsvalue==RS_UNKNOWN || pi_rsvalue==RS_ALL)  
						continue;	

					size_t selected_packs_orig_size = selected_packs_orig.size();
					for (size_t p=0; p<selected_packs_orig_size; p++) {
						selected_packs_set.insert(selected_packs_orig[p]);
					}
					for (std::set<int>::iterator it=selected_packs_set.begin(); it!=selected_packs_set.end(); ++it) {
						selected_packs.push_back(*it);
					}

					if(selected_packs.size()<1) {
						mind->Empty(dim);
						continue;
					}
					else {
						...
						uint seli=0;
						int maxsel=selected_packs.size();
						//mit中的记录完全包含 selected_packs中的数据，用selected_packs来过滤mit
						while(mit.IsValid()) {
							...
							// fix dma-800:reset correct pack if curpack>selected_packs[i]
							while(seli<maxsel && curpack>selected_packs[seli]) ++seli;
							// selected_packs[i]是当前等待匹配的pack
							if(seli>=maxsel || curpack!=selected_packs[seli] ) {
								mit.ResetCurrentPack();
							}
							// skip duplicated packs.
							else while(seli<maxsel && curpack==selected_packs[seli] )  ++seli;
							mit.NextPackrow();
						}
					}
					// commit update
					mit.Commit();
					if(rccontrol.isOn()) {
						rccontrol.lock() <<"PackIndex on Desc ("<<i<< ") got "<<selected_packs.size()<< " Packs ." << unlock;
					}
					Filter* loc_f = mind->GetFilter(dim);
					rough_mind->UpdateGlobalRoughFilter(dim, loc_f);	// if the filter is nontrivial, then copy pack status
					descriptors[i].UpdateVCStatistics();
					descriptors[i].SimplifyAfterRoughAccumulate();					// simplify tree if there is a roughly trivial leaf
			}
		}
