...
bool PackOrderer::Init(vector<VirtualColumn*> vcs, vector<OrderType> orders, int no_unsorted_cols, RSValue* r_filter)
{
	if(Initialized())
		return false;
	...
	if(vcs.size() > 1)
		visited = auto_ptr<Filter>(new Filter(dimsize,nfblocks));
	...
	for(int i = 0; i < vcs.size(); i++) {
		...
		InitOneColumn(vcs[i], orders[i], r_filter, os);
		if(i > 0)
...
void PackOrderer::InitOneColumn(VirtualColumn* vc, OrderType otype, RSValue* r_filter, struct OrderStat os) {
	...
	packs_one_col_begin += (_int64)(packs_one_col.size() * (sorted_percentage/100.0));		// ordering on just sorted_percentage% of packs
...
int PackOrderer::Next(int n) {
	if(curndx[curvc] < 0 ) 
		return -1;
	if(natural_order[curvc]) 
		return (curndx[curvc]+n<dimsize - 1)?(curndx[curvc]+n):-1;
	if(curndx[curvc]+n<packs[curvc].size()-1)
		return packs[curvc][curndx[curvc]+n].second;
	return -1;
}
...
void PackOrderer::RewindToMatch(VirtualColumn* vc, MIIterator& mit)
{
	...
	if(mmtype[curvc] == MMT_Fixed) {
		_int64 min = vc->GetMinInt64(mit);
		_int64 max = vc->GetMaxInt64(mit);
		_int64 mid = min == NULL_VALUE_64 ? MINUS_INF_64 : (max-min)/2;
 ...
 *  ...
 */
void PackOrderer::ChoosePackOrderer(PackOrderer& po, const vector<OrderingInfo>& oi)
{
	...
	po.Init(vcs, orders, unsorted_cols);
}
