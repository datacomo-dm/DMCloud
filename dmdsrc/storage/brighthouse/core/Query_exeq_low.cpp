...
void ParameterizedFilter::SyntacticalDescriptorListPreprocessing(bool for_rough_query)
{
	...
	if(!false_desc) {
		{
			// sasu-276: transaction reference in sub-thread.
			// can not create virtual-column in another thread
			
			//DMThreadGroup ptlist;
			//ptlist.SetThreadKey(ConnectionInfoOnTLS.GetKey());
			//ptlist.SetThreadSpecific((void *)&ConnectionInfoOnTLS.Get());
			for(uint i = 0; i < no_desc; i++) {
				...
				if(descriptors[i].IsOuter())
					additional_nulls_possible = true;
				//ptlist.LockOrCreate()->Start(boost::bind(&ConditionEncoder::EncodeIfPossible,descriptors[i], for_rough_query, additional_nulls_possible,(Transaction *)NULL)));
				ConditionEncoder::EncodeIfPossible(descriptors[i], for_rough_query, additional_nulls_possible,NULL);
			}
			//ptlist.WaitAllEnd();
		}
		for(uint i = 0; i < no_desc; i++) {
