...
void MIUpdatingIterator::PrepareUpdater()
{
	...
	if(one_filter_dim == -1) {
		...
		multi_dim_filter = new Filter(no_obj,nfblocks);
...
void MIUpdatingIterator::ResetCurrentPackBut(CRResultPtr &list)
{
	changed = true;
	if(one_filter_dim > -1)
		one_filter_it->ResetCurrentPackrowBut(list);
	else {
		// should never go here!
		assert(0);
		//multi_dim_filter->Commit();
		//multi_dim_filter->ResetBetweenBut(multi_filter_pack_start, multi_filter_pos + pack_size_left - 1,list);
	}
}

void MIUpdatingIterator::Commit()
{
	if(!changed)
		return;
	if(one_filter_dim > -1) {
...
Filter *MIUpdatingIterator::NewPackFilter(int pack)			// create a new filter for the snapshot and initialize it
{
	...
	Filter *new_f = new Filter(f_old->NoObjOrig(),f_old->GetNFBlocks());
	new_f->Or(*f_old, pack);
	return new_f;	
}
