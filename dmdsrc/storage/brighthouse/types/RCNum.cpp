...
{
	if(!dbl) {
		if((value % Uint64PowOfTen(m_scale)) != 0)
			return false;
		return true;
	}
	return false;
...
RCBString& RCNum::ToRCString(RCBString& rcs) const
{
	if(!IsNull()) {
		char buf[24];
		if(ATI::IsRealType(attrt)) {
			//snprintf(buf, (attrt == RC_REAL ? 15 : 6) + 1, "%.15f",*(double*)&value);
			...
			if(s && buf[s - 1] == '.')
				buf[s - 1] = 0;
			rcs = RCBString((const char *)buf, (int)0, true);
		} else if(ATI::IsIntegerType(attrt))
			sprintf(buf, "%lld", value);
		else {
			return rcs = RCBString((const char *)Text(value, buf, m_scale), (int)0, true);
		}
		rcs.len = (int)strlen(buf);
	}
	else
		rcs = RCBString();
	return rcs;
...
int RCNum::compare(const RCNum& rcn) const
{
	...
	if(IsReal() || rcn.IsReal()) {
		...
		if(m_scale != rcn.m_scale) {
			if(m_scale < rcn.m_scale) {
				_int64 tmpv = (_int64)(value * PowOfTen(rcn.m_scale - m_scale));
				return (tmpv > rcn.value ? 1 : (tmpv == rcn.value ? 0 : -1));
			} else {
				_int64 tmpv = (_int64)(rcn.value * PowOfTen(m_scale - rcn.m_scale));
				return (value > tmpv ? 1 : (value == tmpv ? 0 : -1));
			}
		} else
