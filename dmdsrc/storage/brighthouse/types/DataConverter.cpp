...
RCBString& DC_TxtOracle::Convert(const RCDateTime& dt, RCBString& rcs)
{
	...
	switch(dt.Type())
	{
		...
		case RC_TIME :
		{
			sprintf(buf, "%02d:%02d:%02d", abs((int)dt.Hour()), abs((int)dt.Minute()), abs((int)dt.Second()));
			break;
		}
		case RC_DATETIME  :
		case RC_TIMESTAMP :
		{
			sprintf(buf, "%02d/%02d/%04d %02d:%02d:%02d",abs((int)dt.Month()), abs((int)dt.Day()), abs((int)dt.Year()), abs((int)dt.Hour()), abs((int)dt.Minute()), abs((int)dt.Second()));
			break;
		}
		default :
