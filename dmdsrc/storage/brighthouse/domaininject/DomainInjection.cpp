...
void StringDataBlock::Compress(uint& buf_size)
{
	...
	if(max_len > 0) {	
		...
		nc.Compress(comp_len_buf.get() + 2 * sizeof(uint), comp_len_buf_size, lens.get(), nobj, max_len);
		...
		*(uint*)(comp_len_buf.get() + sizeof(uint)) = max_len;
		...
		TextCompressor tc;			
		tc.Compress(comp_data_buf.get(), comp_data_buf_size, index.get(), lens.get(), nobj, cond ? 0 : TextCompressor::VER);		
...
void StringDataBlock::Decompress(char* src_buf, uint src_buf_size)
{
	...
	if(max_len > 0) {
		...
		nc.Decompress(lens.get(), src_buf + shift, comp_len_buf_size, nobj, (ushort) max_len);
		TextCompressor tc;		
		data = MMGuard<char>((char*) alloc(data_size, BLOCK_TEMPORARY), *this);
		if(nobj)
			tc.Decompress(data.get(), data_size, src_buf + shift + comp_len_buf_size, src_buf_size - shift - comp_len_buf_size, index.get(), lens.get(), nobj);
	} else {
		for(int i = 0; i<nobj; i++) {
			index.get()[i] = NULL;
			lens.get()[i] = 0;
