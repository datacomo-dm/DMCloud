		...
		if (outlier) { 
			...
			for(int bi = 0; bi < no_bl_n + no_bl_s; bi++) {
				if (m_block_type.at(bi) == MASK_BLOCK_N) {
					block_n.at(bl_n_cnt++)->Add(parseNumber(subgr[bi + 1].first, subgr[bi + 1].length()));
				} else {
					block_s.at(bl_s_cnt++)->Add(subgr[bi + 1].first, subgr[bi + 1].length());
				}				
			}
			outl_mask->Add(0);
		}
			...
			if( ((NumericDataBlock<uchar>*) (in[0].get()))->GetValue(o) == 0 ) { // not outlier	
				...
				while (mask_cnt < mask.length()) {
					if (mask[mask_cnt] != '%' || mask_cnt+1==mask.length() || (mask[mask_cnt+1]!='s' && mask[mask_cnt+1]!='d')) {
						...
						if (m_block_type.at(block_cnt) == MASK_BLOCK_N) {
							int licz  = int(((NumericDataBlock<_uint64>*) (in[block_cnt + 2].get()))->GetValue(parsed_cnt));
							itoa(licz, tmp_s, 10);
							uchar dig = uchar(strlen(tmp_s));
		...
		for(int o = 0; o < nobj; o++) {	
			...
			while (mask_cnt < mask.length()) {
				if (mask[mask_cnt] != '%' || mask_cnt+1==mask.length() || (mask[mask_cnt+1]!='s' && mask[mask_cnt+1]!='d')) {
					...
					if (m_block_type.at(block_cnt) == MASK_BLOCK_N) {
						uint licz  = uint(((NumericDataBlock<_uint64>*) (in[block_cnt].get()))->GetValue(o));
						itoa(licz, tmp_s, 10);
						uchar dig = uchar(strlen(tmp_s));
						memcpy(data_ + shift + len, tmp_s, dig);						
						len += dig;
