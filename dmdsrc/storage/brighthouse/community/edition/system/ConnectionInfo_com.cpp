...
void IBTableShared::ReleaseLock(TableLockWeakPtr lock)
{
	...
	if(tlock->IsWriteLock()) {
		...
		BHASSERT_WITH_NO_PERFORMANCE_IMPACT(find_if(read_lock_list.begin(), read_lock_list.end(), bl::_1 == tlock) != read_lock_list.end());
		read_lock_list.erase(find_if(read_lock_list.begin(), read_lock_list.end(), bl::_1 == tlock));
		//for_each(scheduled_jobs.begin(), scheduled_jobs.end(), bind(&SchedulerJob::RemoveLock, _1, tlock));
		//RunScheduledJobs();
	}
}
...
void IBTableShared::ReleaseLocks(const TransactionBase& trans)
{
	...
	//TableLockPtr tl(new TableLock(table_path, boost::ref(trans), F_RDLCK));
	//for_each(scheduled_jobs.begin(), scheduled_jobs.end(), bind(&SchedulerJob::RemoveLocks, _1, tl));
	//RunScheduledJobs();
}
