...
private:
    ...
    IBThread flush_thread;
    IBThread read_thread;
...
private:
#ifdef __GNUC__
	static void *ReadThread(void* lb);
#else
    static DWORD WINAPI ReadThread(void* lb);
#endif
	void UseNextBuf();
	int FindUnusedBuf();
...
#ifdef __GNUC__
	static void *BufFlushThread(void* _pars);
#else
    static DWORD WINAPI BufFlushThread(void* _pars);
#endif 
};
