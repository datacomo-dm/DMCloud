...

{
	...
	int cr_debug_level = CR_Class_NS::get_debug_level();

	if ((cr_debug_level >= 30) && (hpolicy.compare("") == 0)) {
		DPRINTF("cr_debug_level[%d]\n", cr_debug_level);
		hpolicy = "system";
	}
	...
	rccontrol << lock << "Heap Policy: " << hpolicy << unlock;
...
MEM_HANDLE_MP MemoryHandling::alloc(size_t size, BLOCK_TYPE type, TrackableObject* owner, bool nothrow)
{
	...
	HeapPolicy *heap;
	// change maximum buffer size from 2GB 1<<31 to 8GB 4<<31
	BHASSERT(size < ((long)4<<31), "Oversized alloc request!");
	...
	if( res == NULL ) {
		...
		if( res == NULL )  {
			...
			if( res == NULL ) {
				if (nothrow) return res;
				rccontrol << lock << "Failed to alloc block of size " << static_cast<int>(size) << unlock;
				throw OutOfMemoryRCException();
			} else {
				heap = m_system;
			}
...
void MemoryHandling::clear_freelist(MEM_HANDLE_MP mh, TrackableObject* owner) {
    IBGuard guard(m_mutex);
	
	if(mh == NULL) return;
	
	OwnerMemoryMap::iterator it = m_objs.find(owner);
	BHASSERT( it != m_objs.end(), "clear_section owner not found" );

	PtrHeapMap::iterator h = it->second->find(mh);
	BHASSERT( h != it->second->end(), "clear_section  heap not found." );
	h->second->clear_freelist(mh);
}
...
bool MemoryHandling::ReleaseMemory(size_t size, RELEASE_HEAP rh, TrackableObject* untouchable)
{
	...
	m_release_count++;
	// packs allocated in m_main_heap ,unless alloc failed again,so skip this ,ref sasu-269.
