...
HugeHeap::HugeHeap(std::string hugedir, size_t size ) : TCMHeap(0)
{
	...
	if (!hugedir.empty() && size > 0) {
		char pidtext[12];
		dm_strlcpy(m_hugefilename, hugedir.c_str(), sizeof(m_hugefilename));
		sprintf(pidtext, "%d", getpid());
		dm_strlcat(m_hugefilename, "/bhhuge.", sizeof(m_hugefilename));
		dm_strlcat(m_hugefilename, pidtext, sizeof(m_hugefilename));
