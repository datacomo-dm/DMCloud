...
void TrackableObject::clear_freelist(void *ptr) {
	size_t s;
	if( ptr == NULL ) return;
	Instance()->clear_freelist(ptr, this);
}
...
{			
	_int64 max_total_size;								// NOTE: must be a power of 2
	...
	else if(mem_scale <= 2)		// normal settings (up to 2.5 GB is reserved)
		max_total_size =  128 * MBYTE;
	else if(mem_scale == 3)		// high settings (when 2.5 - 5 GB is reserved in 64-bit version)
		max_total_size =  256 * MBYTE;
	else if(mem_scale == 4)		// high settings (when 5 - 10 GB is reserved in 64-bit version)
		max_total_size =  512 * MBYTE;
	else					// high settings (when at least 10 GB is reserved in 64-bit version)
