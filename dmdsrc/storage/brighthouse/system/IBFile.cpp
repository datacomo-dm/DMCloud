	...
	throw(DatabaseRCException)
{
	...
	if(timeout == IBFILE_OPEN_NO_TIMEOUT) {
		...
		do {
			...
			if(fd == -1) {
				IBThread::SleepNow(IBFILE_OPEN_SLEEP_STEP);
...
#endif
    if(auto_exception && (writen_bytes!=count)){
		char const eMsg[] = "IBFile::Write there is insufficient space on the underlying physical medium, or the RLIMIT_FSIZE resource limit is encountered (see setrlimit(2)).";
	...
	throw(DatabaseRCException)
{
	...
	int id = static_cast<int>( reinterpret_cast<int long>(reinterpret_cast<void*>(IBThread::GetSelf()))); //pthread_self();
	...
	throw(DatabaseRCException)
{
	...
	if ((int) read_bytes != count) {
		stringstream ss;
		//raise(SIGABRT);  // try to fix dma-1227
	...
	throw(DatabaseRCException)
{
	...
	//if ((int) writen_bytes == 0 && count != 0)
	//	ThrowError("wrote only 0 bytes");
	//else if ((int) writen_bytes != count)
	if ((int) writen_bytes != count)
