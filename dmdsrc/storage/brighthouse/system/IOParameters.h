...
public:
	...
	void SetParameter(Parameter param, int value)
	{
		switch(param)
		{
		case DELIMITER			: delimiter = (char)value; break;
	...
	void SetNullsStr(std::string& null_str)
	{
	...
	char		Delimiter() const		{ return delimiter; }
	//std::string	Delimiter() const		{ return delimiter; }
	...
	std::string	LineTerminator() const	{ return line_terminator; }
	...
	*/
	void PutParameters(IBFile& stream);
	void GetParameters(IBFile& stream);
