    ...
    char* filefirst)
{
    ...
    dm_strlcpy(tmp_path, srcdir, sizeof(tmp_path));
    dm_strlcat(tmp_path, DIR_SEPARATOR_STRING, sizeof(tmp_path));
...
// force rename file even newfile exists
void RenameFileF(string const &oldfile,string const &newfile)
	throw(DatabaseRCException)
{
	if(!DoesFileExist( oldfile)) return;
