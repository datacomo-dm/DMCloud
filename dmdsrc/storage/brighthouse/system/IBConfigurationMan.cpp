...
int bh_sysvar_parscan_maxscanthreads;
...
int IBConfigurationManager::LoadConfig(std::string &error)
{
	...
	// root->version
	root->AddChild(new NodeElement("version", "1.0b", ELEMENT_TYPE_STRING, ""));
...
void IBConfigurationManager::PublishElementsToShowVar()
{
	...
	bh_sysvar_parscan_maxscanthreads = GetValueInt("brighthouse/parallelscan/maxscanthreads", error);
