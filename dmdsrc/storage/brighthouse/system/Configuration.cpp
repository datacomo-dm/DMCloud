...
INIT_BOOST_PROPERTY(MemoryManagementPolicy, string, "");
INIT_BOOST_PROPERTY(MemoryManagementReleasePolicy, string, "");
...
void Configuration::LoadSettings(std::string configuration_filename)
{
	...
	if(!configuration_file) {
		...
		char buf[1000];
		snprintf (buf, sizeof(buf), "ERROR: The configuration file %s not found", configuration_filename.c_str());
		throw Exception(buf);
	}
	else {
		try {
			_loaded = true;
			rccontrol << lock << "Loading configuration for DataMergerD 1.0b instance ..." << unlock;
			...
			char buf [1000];
			snprintf(buf, sizeof(buf), "ERROR: Failed to parse %s, parsing error = %s",  configuration_filename.c_str(), e.what());
			throw Exception(buf);
		}
	}
