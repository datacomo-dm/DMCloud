...
bool LargeBuffer::StartReadingThread()
{
	if(BufferingLevel > 0)
	{
        ...
        read_thread.Start(ReadThread, (void*) this);
		threadId = read_thread.GetThreadID();
...
struct FlushThreadParams
{
	FlushThreadParams(IBStream *_file, char* _buf, int _len, bool& _failed)
       : file(_file), buf(_buf), buf_used(_len), failed(&_failed) {}
	IBStream *file;
    char* buf;
    int buf_used;
	bool* failed;
...
void LargeBuffer::BufFlush()		// save the data to the file (in write mode)
{
	...
	THREADVAR threadId;
	FlushThreadParams *ftp = new FlushThreadParams(curfile.get(), buf, buf_used, failed);
	flush_thread.Start(BufFlushThread, (void*)ftp);
	threadId = flush_thread.GetThreadID();
	UseNextBuf();
}

#ifdef __GNUC__
void *LargeBuffer::BufFlushThread(void* _pars)
#else
DWORD WINAPI LargeBuffer::BufFlushThread(void* _pars)
#endif
{
  FlushThreadParams *ftp = (FlushThreadParams*) _pars;
	IBStream *file = ftp->file;
	char* buf_ptr = ftp->buf;
	int len = ftp->buf_used;
	bool* failed = ftp->failed;
  delete ftp;
...
{
	// JIRA DMA-87: unused_bytes is negative integer.
	if(to_read == 0)
		to_read = size;
	if(buf_status == 1) {
		...
		if(curfile->IsOpen()) {
			...
			while(no_steps < 15 && to_read > 0) {
				...
				if(r != -1) {
					...
					IBThread::SleepNow(500 + no_steps*500);
				}
...
#ifdef __GNUC__
void *LargeBuffer::ReadThread(void* _lb)
#else
DWORD WINAPI LargeBuffer::ReadThread(void* _lb)
#endif
{
	int to_read = 0;
	LargeBuffer* lb = (LargeBuffer*)_lb;
	int to_read_in_one_loop = lb->size / BufferingLevel;
	bool do_sleep = false;
	bool do_stop = false;
	...
	while(!do_stop) {
		no_read_bytes = 0;
        lb->read_mutex.Lock();
		if(lb->buf_incomplete != 1 || !lb->curfile->IsOpen() || lb->stop_reading_thread) {
            lb->read_mutex.Unlock();
			break;
		}

		if(lb->bytes_in_read_thread_buffer != lb->size) {
			to_read = to_read_in_one_loop;
			if(lb->size - lb->bytes_in_read_thread_buffer < to_read_in_one_loop)
				to_read = lb->size - lb->bytes_in_read_thread_buffer;
			ushort no_steps = 0;
			while(no_steps < 15  && to_read > 0) {
				try {
				no_read_bytes = lb->curfile->Read(lb->read_thread_buffer + lb->bytes_in_read_thread_buffer, to_read);
				} catch(DatabaseRCException&) {
					no_read_bytes = -1;
				}				
				if(no_read_bytes == -1) {
					...
					do_stop = false;
					lb->bytes_in_read_thread_buffer += no_read_bytes;
					if(no_read_bytes != 0)
						lb->buf_incomplete = 1;
					else {
						lb->curfile->Close();
			...
			if(no_read_bytes == -1) {
				lb->bytes_in_read_thread_buffer = -1;
				do_stop = true;
			}
		}
