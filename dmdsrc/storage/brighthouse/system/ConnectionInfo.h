...
public:
	...
	void AddHoldTable(BrighthouseHandler *phandle);
	void RemoveHoldTable(BrighthouseHandler *phandle);

	void ClearHoldTable() ;
	
private:
	std::set<BrighthouseHandler *> hold_tables;
    IBMutex htmx;                   // hold_tables สนำร
...
public:
	...
	virtual void Minus(const SchedulableJob& job) = 0;
	virtual void And(const SchedulableJob& job) = 0;
};

typedef boost::shared_ptr<SchedulableJob> SchedulableJobPtr;

class SchedulerJob {
public:
	SchedulerJob(TablesLocksList locks_list, SchedulableJobPtr job)
		: locks_list(locks_list), the_job(job)
	{
	}

	void	RemoveLock(TableLockPtr lock);
	void 	RemoveLocks(TableLockPtr lock);
	size_t	NoLocks() const { return locks_list.size(); }

public:
	TablesLocksList		locks_list;
	SchedulableJobPtr	the_job;
};

typedef boost::shared_ptr<SchedulerJob> SchedulerJobPtr;
...
#endif
	std::vector<boost::shared_ptr<SchedulerJob> > scheduled_jobs;
};
...
public:
	 ...
	 */
	void ReleaseLockIfNeeded(TableLockWeakPtr table_lock, bool is_autocommit_on, bool in_alter_table);
	...
	void ScheduleJob(const std::string& table_path, SchedulableJobPtr job);
	//bool HasReadLockForOtherTransaction(const std::string& table_path, const TransactionBase& trans);
