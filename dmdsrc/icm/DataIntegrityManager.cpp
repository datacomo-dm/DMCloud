...
{	
	...
	if (cnt_fails == 0)
		...
		for(vector<DataIntegrityTest*>::iterator it = DIMParameters::failed_tests.begin(); it!=DIMParameters::failed_tests.end(); ++it) {
			WriteToLog(string("db: ") + (*it)->GetDatabaseName() + string(" tab: ") + (*it)->GetTableName() + " test: " + (*it)->GetTestName());
		}
