...
test ! -z "`uname -a|egrep -i -e SunOS -e solaris`" && SED="/usr/local/bin/sed"
...
getcpucount()
{
  [ ! -z "`uname -a|egrep -i -e SunOS`" -o ! -z "`uname -a|egrep -i -e solaris`" ] && processors=`/usr/sbin/prtconf -v|grep cpu-model|wc -l`
