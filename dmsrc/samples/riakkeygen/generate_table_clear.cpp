                        ...
                        stru_partinfo& st_partition_info)
{
    ...
    int match_syninit_synwait = 0;       // 判断syninit和synwait 函数调用是否匹配
            ...
            if(ret == 1){// 读取成功，判断是否是有效包
                ...
                if(is_stored){
                    ...
                    if(strlen(st_dst_table_assist._s_d_p_item.fcurrent_part_name) == 0 ){ // 第一个分区的开始转换
                        ...
                        match_syninit_synwait++;                        
                        assert(match_syninit_synwait == 1);
                        ...
                        // 提交上一个分区的装入文件
                        {
                            ...
                            match_syninit_synwait--;
                            assert(match_syninit_synwait == 0);
                    ...
                    {    
                        ...
                        // 加载 packn/packs
                        {
                            ...
                            if(packtype == PackN) {// read PackN and send to riak
                                ...
                                if(ib_packn_compressmode_assist::IsModeNoCompression(optimal_mode)) { // 不需要进行压缩的数据包
                                ...
                                } else { // 压缩过的数据，直接将压缩包发送到riak-proxy中处理
                                    if(total_size < 17) {
                                    ...
                                    // 发送整个数据包到riak中
                                    std::string put_param;
                                    put_param = CR_Class_NS::str_setparam(put_param, RIAK_META_NAME_SAVE_COMPRESS, RIAK_META_VALUE_FALSE);
                                    
                                    AttrPackN_Save_2(no_nulls,(const char*)compressed_buf,comp_buf_size,no_obj_in_pack,optimal_mode,local_min,max_val,str_riak_key_value,put_param);
                                }
                                ...
                                if(!ib_packs_compressmode_assist::IsModeValid(optimal_mode)) {
                                    ...
                                    if(ib_packs_compressmode_assist::IsModeNoCompression(optimal_mode)){ // 没有压缩的,直接读取即可
                                        ...
                                        if(ib_packs_compressmode_assist::IsModeCompressionApplied(optimal_mode)){ // 0,1    = Huffmann 16-bit compression of remainings with tree (0 - flat null encoding, 1 - compressed nulls)
                                            ...
                                            // 设置成riak接口接口向riak中发送
                                            AttrPackS_Save_2(_no_nulls,(const char*)compressed_buf,comp_buf_size,data_full_byte_size,len_mode,decomposer_id,no_groups,no_obj_in_pack,optimal_mode,ver,str_riak_key_value,put_param);
                    ...
                    const std::string& riak_params) /*: 压缩类型参数 */
{
    ...
    if(compressed_buf){
        msg_pack.set_compressed_buf((void*)compressed_buf, compressed_buf_len);
    }

    msg_pack.SerializeToString(&riak_value);

#ifdef PRINT_LOG_INFO
    lgprintf("AttrPackN_Save_2, riakkey = %s , value_size = %d\n", str_riak_key_node.c_str(), (int)(riak_value.size()));
#endif
...
int main(int argc,char *argv[]) {
    WOCIInit("generate_table/generate_table_clear_");
    ...
    if(argc != 6){
        ...
        lgprintf("参数说明: ");
        lgprintf("        ./generate_table_clear 源数据库数据路径  源数据库名称 源数据库表名称 目标数据库数据路径 目标数据库名称 ");
        ...
        lgprintf("样例:");
        lgprintf("        ./generate_table_clear /home/dma_new/dma/var dbtest dp_datapart /home/dma_new/dma_riak_test/var dbtest");
        WOCIQuit(); 
        return -__LINE__;
    }
