...
void DbplusCert::GetExpiredDate(int &y,int &m,int &d)
{
    ...
    if(NULL != pRiakHost && pRiakHost[0] != '\0') {
        const char *fc=getLine("expiredate_riak");
        strcpy(_tmp_date,fc);
    } else {
        const char *fc=getLine("expiredate");
        strcpy(_tmp_date,fc);
    }
    const char* fc = _tmp_date;
...
const char *DbplusCert::getVersion(int versiontype)
{
    switch(versiontype) {
        case 1:
            return getLine("core_version");
        case 2:
            return getLine("bs_version");
        case 3:
            return getLine("admin_version");
        case 4:
            return getLine("lib_version");
    }
    return NULL;
}
...
int DbplusCert::DecodeFromFile(const char *fn)
{
    ...
    if(strncmp(getVersion(4),GetDPLibVersion(),2)!=0) {
        ThrowWith("授权证书版本错误.\n");
        return -4;
    }
    return len/128;
