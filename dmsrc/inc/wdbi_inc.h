...
//////////////////////////////////////////////////////////////////
#ifndef WOCI_DEBUG
#define wociGetMaxSession()							_wdbiGetMaxSession()
#define wociGetMaxStatment()						_wdbiGetMaxStatment()
#define wociGetMaxMemTable()						_wdbiGetMaxMemTable()
#define wociPrintfVersion()							_wdbiPrintfVersion()
...
#define wociGetStmtColumnSize(stmt,col)                                            _wdbiGetStmtColumnSize(stmt,col)
#define wociSetStmtColumnSize(stmt,col,size)                                       _wdbiSetStmtColumnSize(stmt,col,size)
#define wociGetCreateTableSQL(memtab,buf,tabname,ismysql)                          _wdbiGetCreateTableSQL(memtab,buf,tabname,ismysql)                     
#define wociGetCreateTableColumnCommentSQL(memtab,crttab,tabname,stColumComment,columnsNum,ismysql)  _wdbiGetCreateTableColumnCommentSQL(memtab,crttab,tabname,stColumComment,columnsNum,ismysql)
...
// 获取整行数据，返回字符串add by lirawposujs
...
#define wociWaitStmt(stmt)                                                         _wdbiWaitStmt(stmt)                                                          
#define wociWaitMemtable(memtab)                                                   _wdbiWaitMemtable(memtab)                                                    
...
// 导出文本文件*.cvs,add by liujs 
#define wociMTToTextFileStr(memtab,fn,rownm,colsnm,title)                          _wdbiMTToTextFileStr(memtab,fn,rownm,colsnm,title)    
...
#define wociConvertColStrToInt(memtab,colsn,pcol)                                  _wdbiConvertColStrToInt(memtab,colsn,pcol,NULL)                                   
#define wociConvertColStrToInt2(memtab,colsn,pcol,ppartlen)                        _wdbiConvertColStrToInt(memtab,colsn,pcol,ppartlen)                                   
...
#define wociCopyToIBBinary(memtab,startrow,rownum,fp)                              _wdbiCopyToIBBinary(memtab,startrow,rownum,fp)                                  
#define wociCopyToIBBuffer(memtab,startrow,buf,maxbuflen,actual_len)               _wdbiCopyToIBBuffer(memtab,startrow,buf,maxbuflen,actual_len)                                  
...
#define wociGetMaxSession()							(LOGWHERE,_wdbiGetMaxSession())
#define wociGetMaxStatment()						(LOGWHERE,_wdbiGetMaxStatment())
#define wociGetMaxMemTable()						(LOGWHERE,_wdbiGetMaxMemTable())
#define wociPrintfVersion()							(LOGWHERE,_wdbiPrintfVersion())
...
#define wociGetStmtColumnSize(stmt,col)                                            (LOGWHERE,_wdbiGetStmtColumnSize(stmt,col)                                             )
#define wociSetStmtColumnSize(stmt,col,size)                                       (LOGWHERE,_wdbiSetStmtColumnSize(stmt,col,size)                                        )
#define wociGetCreateTableSQL(memtab,buf,tabname,ismysql)                          (LOGWHERE,_wdbiGetCreateTableSQL(memtab,buf,tabname,ismysql)                           )
#define wociGetCreateTableColumnCommentSQL(memtab,crttab,tabname,stColumComment,columnsNum,ismysql)  (LOGWHERE,_wdbiGetCreateTableColumnCommentSQL(memtab,crttab,tabname,stColumComment,columnsNum,ismysql))
...
#define wociWaitStmt(stmt)                                                         (LOGWHERE,_wdbiWaitStmt(stmt)                                                          )
#define wociWaitMemtable(memtab)                                                   (LOGWHERE,_wdbiWaitMemtable(memtab)                                                    )
...
// 导出文本文件*.cvs ,add by liujs     
#define wociMTToTextFileStr(memtab,fn,rownm,colsnm,title)                          (LOGWHERE,_wdbiMTToTextFileStr(memtab,fn,rownm,colsnm,title)                           )                        
...
#define wociConvertColStrToInt(memtab,colsn,pcol)                                  (LOGWHERE,_wdbiConvertColStrToInt(memtab,colsn,pcol,NULL)                              )
#define wociConvertColStrToInt2(memtab,colsn,pcol,ppartlen)                      (LOGWHERE,_wdbiConvertColStrToInt(memtab,colsn,pcol,ppartlen)                        )
...
#define wociCopyToIBBinary(memtab,startrow,rownum,fp)                              (LOGWHERE,_wdbiCopyToIBBinary(memtab,startrow,rownum,fp)                               )
#define wociCopyToIBBuffer(memtab,startrow,buf,maxbuflen,actual_len)               (LOGWHERE,_wdbiCopyToIBBuffer(memtab,startrow,buf,maxbuflen,actual_len)                )
...
DllExport void _wdbiPrintfVersion();

DllExport int _wdbiGetMaxSession();
DllExport int _wdbiGetMaxStatment();
DllExport int _wdbiGetMaxMemTable();
...
DllExport int _wdbiGetCreateTableColumnCommentSQL(int memtab,char *crttab,const char *tabname,const void* stColumComment,const int columnsNum,bool ismysql);
...
DllExport  int _wdbiGetStmtColumnSize(int stmt,int col);
DllExport  bool _wdbiSetStmtColumnSize(int stmt,int col,int size);
//////////////////////////////////////////////////////////////////
...
//获取定长地址，只能读不能写
DllExport  const char *_wdbiGetVarLenStrAddrByName(int memtab,const char *col,unsigned int rowst,int *celllen);
...
DllExport int _wdbiGetStrValByPos(int memtab,int col,unsigned int rowst,char *pstr,int rowNm=1);
...
DllExport  const char*  _wdbiGetVarStrAddrByPos(int memtab,int col,unsigned int rowst,int *celllen) ;
...
DllExport  long  _wdbiExportSomeRows(int memtab,char *pData,int startrn,int rnnum);
DllExport  bool _wdbiExport(int memtab,void *pData,long &_len,int &_maxrows,int &_rowct) ;
DllExport bool _wdbiImport(int memtab,void *pData,long _len,void *pColDesc,int _colnm,int _cdlen,int _maxrows,int _rowct);
DllExport  long  _wdbiAppendRows(int memtab,char *pData,int rnnum); 
 	//保留表结构，清除索引结构，数据行数置零
...
//  导出文本文件*.cvs,add by liujs
DllExport void _wdbiMTToTextFileStr(int memtab,const char *fn,int rownm,const char *colsnm,const bool title);
...
DllExport int lgfilename(char* lgfn,int &lgfnlen);
...
DllExport int _wdbiConvertColStrToInt(int memtab,const char *colsn,int 
*pcol,int *partlen=NULL);
...
DllExport void _wdbiCopyToIBBinary(int memtab,unsigned int startrow,
			unsigned int rownum,FILE *fp);
DllExport int _wdbiCopyToIBBuffer(int memtab,unsigned int startrow,
			char *buf,int maxbuflen,int *actual_len);
...
// Get wheach col is var Len and it's lenth
DllExport int _wdbiSetVarLenLimit(int memtab,int nVarLen);
DllExport int _wdbiGetVarCol(int memtab,int *pCol,int *pTypeLen);
DllExport int _wdbiCopyVarStrColDataInTable(int memtab,int nCol,long nToRow,long nFromRow,long nCopyRowNum);
DllExport BOOL _wdbiCheckData(int memtab, int nCol); //调试使用函数
DllExport bool _wdbiDeleteToDbTableWithPK(int memtab,const char *tablename,int sess,bool autocommit);
DllExport bool _wdbiSetBindNullFlagForOutMem(int memtab,const int stmt,int pos,int colid,int rowst);
DllExport int _wdbiSearchSortedRows(int memtab,void **ptr,int &nStartSortRowNum,int &nEndSortRowNum ,int nReallSortCol= -1);    

//Backup AVL Sort  Data
DllExport int   _wdbiGetSortAVLLen(int memtab);
DllExport bool  _wdbiBackupSortAVL(int memtab,char *pBuff,int nLen);
DllExport bool  _wdbiSetAVLSort(int memtab,char *pBuff,int nNodeNum);

//QDelete Export And Import
DllExport int   _wdbiExportQDeleteFlag(int memtab,char *pData ,int &nDeleteRowct);
DllExport void  _wdbiImportQDeleteFlag(int memtab,char *pData , int nDeleteRowct);

//Set QDel Data
DllExport void _wdbiSetQDeleteFlagFalse(int memtab, int nRow);
DllExport int _wdbiSearchNoQDel(int memtab,void **ptr);
